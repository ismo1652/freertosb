...
&
THEORY ListSeesX IS
  List_Sees(Refinement(Task_r))==(FreeRTOSConfig,Types,Scheduler)
END
&
...
THEORY ListVariablesX IS
  ...
  Abstract_List_Variables(Refinement(Task_r))==(idle,running,suspended,runable,blocked,tasks,active);
  Local_List_Variables(Refinement(Task_r))==(t_bpriority,t_priority,idle,running,suspended,runable,blocked,tasks,active);
  List_Variables(Refinement(Task_r))==(t_bpriority,t_priority,idle,running,suspended,runable,blocked,tasks,active);
  External_List_Variables(Refinement(Task_r))==(t_bpriority,t_priority,idle,running,suspended,runable,blocked,tasks,active)
END
&
...
THEORY ListInvariantX IS
  ...
  Context_List_Invariant(Refinement(Task_r))==(schedulerRunning: BOOL & schedulerSuspended: NATURAL);
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Refinement(Task_r))==(active,tasks:=FALSE,{} || @(running$0).(running$0: TASK ==> running:=running$0) || @(idle$0).(idle$0: TASK ==> idle:=idle$0) || t_priority:={} || t_bpriority:={} || blocked,runable,suspended:={},{},{});
  Context_List_Initialisation(Refinement(Task_r))==(skip);
  List_Initialisation(Refinement(Task_r))==(active:=FALSE || tasks:={} || running:: TASK || idle:: TASK || t_priority:={} || t_bpriority:={} || blocked,runable,suspended:={},{},{})
END
&
THEORY ListOperationsX IS
  Internal_List_Operations(Refinement(Task_r))==(t_create,t_delete,t_yield,t_suspend,t_resume,t_getPriority,t_getCurrent,t_getNumberOfTasks,t_delayTask,t_startScheduler,t_endScheduler,t_resumeAll,t_unblock,t_removeFromEventList,t_setPriority,t_priorityInherit,t_returnPriority,t_suspendAll);
  List_Operations(Refinement(Task_r))==(t_create,t_delete,t_yield,t_suspend,t_resume,t_getPriority,t_getCurrent,t_getNumberOfTasks,t_delayTask,t_startScheduler,t_endScheduler,t_resumeAll,t_unblock,t_removeFromEventList,t_setPriority,t_priorityInherit,t_returnPriority,t_suspendAll)
END
&
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Refinement(Task_r),t_resumeAll)==(unblocked: FIN(TASK) & unblocked <: blocked & running: runable);
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Refinement(Task_r),t_removeFromEventList)==(task: TASK & task: blocked\/suspended & active = TRUE | task: blocked ==> blocked:=blocked-{task} [] not(task: blocked) ==> skip || (task: suspended ==> suspended:=suspended-{task} [] not(task: suspended) ==> skip) || @n_runable.(n_runable <: tasks & n_runable = runable\/{task} ==> (runable:=n_runable || (t_priority(task)>=t_priority(running) ==> @(running$0).(running$0: schedule_p(n_runable,t_priority) ==> running:=running$0) [] not(t_priority(task)>=t_priority(running)) ==> skip))));
  Expanded_List_Substitution(Refinement(Task_r),t_unblock)==(task: TASK & task: blocked & active = TRUE | blocked:=blocked-{task} || @n_runable.(n_runable <: tasks & n_runable = runable\/{task} ==> (runable:=n_runable || (t_priority(task)>=t_priority(running) ==> @(running$0).(running$0: schedule_p(n_runable,t_priority) ==> running:=running$0) [] not(t_priority(task)>=t_priority(running)) ==> skip))));
  Expanded_List_Substitution(Refinement(Task_r),t_resumeAll)==(unblocked: FIN(TASK) & unblocked <: blocked & running: runable | unblocked/={} ==> (@n_runable.(n_runable <: tasks & n_runable = runable\/unblocked ==> (runable:=n_runable || @(running$0).(running$0: schedule_p(n_runable,t_priority) ==> running:=running$0))) || blocked:=blocked-unblocked) [] not(unblocked/={}) ==> skip);
  Expanded_List_Substitution(Refinement(Task_r),t_endScheduler)==(active = TRUE | active,tasks,t_priority,t_bpriority,blocked,runable,suspended:=FALSE,{},{},{},{},{},{});
  ...
  List_Substitution(Refinement(Task_r),t_delete)==(ANY n_runable,n_priority WHERE n_runable <: tasks & n_runable = runable-{atask} & n_runable/={} & n_priority: TASK +-> PRIORITY & n_priority = {atask}<<|t_priority THEN tasks:=tasks-{atask} || t_priority:=n_priority || t_bpriority:={atask}<<|t_bpriority || IF atask: runable THEN runable:=n_runable || IF atask = running THEN IF active = TRUE THEN running:: schedule_p(n_runable,n_priority) END END ELSIF atask: blocked THEN blocked:=blocked-{atask} ELSIF atask: suspended THEN suspended:=suspended-{atask} END END);
  List_Substitution(Refinement(Task_r),t_yield)==(running:: schedule_p(runable,t_priority));
  List_Substitution(Refinement(Task_r),t_suspend)==(IF stask: runable THEN ANY n_runable WHERE n_runable <: tasks & n_runable = runable-{stask} & n_runable/={} THEN runable:=n_runable || IF stask = running THEN running:: schedule_p(n_runable,t_priority) END END ELSIF stask: blocked THEN blocked:=blocked-{stask} END || suspended:=suspended\/{stask});
  ...
  List_Substitution(Refinement(Task_r),t_endScheduler)==(active:=FALSE || tasks:={} || t_priority:={} || t_bpriority:={} || blocked,runable,suspended:={},{},{});
  List_Substitution(Refinement(Task_r),t_resumeAll)==(IF unblocked/={} THEN ANY n_runable WHERE n_runable <: tasks & n_runable = runable\/unblocked THEN runable:=n_runable || running:: schedule_p(n_runable,t_priority) END || blocked:=blocked-unblocked END);
  List_Substitution(Refinement(Task_r),t_unblock)==(blocked:=blocked-{task} || ANY n_runable WHERE n_runable <: tasks & n_runable = runable\/{task} THEN runable:=n_runable || IF t_priority(task)>=t_priority(running) THEN running:: schedule_p(n_runable,t_priority) END END);
  List_Substitution(Refinement(Task_r),t_removeFromEventList)==(IF task: blocked THEN blocked:=blocked-{task} END || IF task: suspended THEN suspended:=suspended-{task} END || ANY n_runable WHERE n_runable <: tasks & n_runable = runable\/{task} THEN runable:=n_runable || IF t_priority(task)>=t_priority(running) THEN running:: schedule_p(n_runable,t_priority) END END);
  ...
  List_Substitution(Refinement(Task_r),t_returnPriority)==(IF btask: blocked THEN blocked:=blocked-{btask} END || IF btask: suspended THEN suspended:=suspended-{btask} END || ANY n_runable,n_priority WHERE n_runable <: tasks & n_runable = runable\/{btask} & n_priority: TASK +-> PRIORITY & n_priority = t_priority<+{running|->t_bpriority(running)} THEN runable:=n_runable || IF t_bpriority(running)/=t_priority(running) THEN t_priority:=n_priority || running:: schedule_p(n_runable,n_priority) ELSIF t_priority(btask)>=t_priority(running) THEN running:: schedule_p(n_runable,t_priority) END END)
END
&
...
THEORY ListInstanciatedParametersX IS
  ...
  List_Instanciated_Parameters(Refinement(Task_r),Machine(Scheduler))==(?)
END
&
...
THEORY ListSeenInfoX IS
  Seen_Internal_List_Operations(Refinement(Task_r),Machine(Scheduler))==(startScheduler,stopScheduler,suspendScheduler,resumeScheduler);
  ...
  Seen_List_Precondition(Refinement(Task_r),resumeScheduler)==(schedulerSuspended>0);
  Seen_Expanded_List_Substitution(Refinement(Task_r),resumeScheduler)==(schedulerSuspended:=schedulerSuspended-1);
  Seen_List_Precondition(Refinement(Task_r),suspendScheduler)==(schedulerSuspended<MAXINT);
  Seen_Expanded_List_Substitution(Refinement(Task_r),suspendScheduler)==(schedulerSuspended:=schedulerSuspended+1);
  Seen_List_Precondition(Refinement(Task_r),stopScheduler)==(schedulerRunning = TRUE);
  Seen_Expanded_List_Substitution(Refinement(Task_r),stopScheduler)==(schedulerRunning:=FALSE);
  Seen_List_Precondition(Refinement(Task_r),startScheduler)==(schedulerRunning = FALSE);
  Seen_Expanded_List_Substitution(Refinement(Task_r),startScheduler)==(schedulerRunning:=TRUE);
  Seen_List_Operations(Refinement(Task_r),Machine(Scheduler))==(startScheduler,stopScheduler,suspendScheduler,resumeScheduler);
  Seen_Expanded_List_Invariant(Refinement(Task_r),Machine(Scheduler))==(btrue);
...
THEORY ListANYVarX IS
  ...
  List_ANY_Var(Refinement(Task_r),t_returnPriority)==((Var(n_runable) == SetOf(atype(TASK,?,?))),(Var(n_priority) == SetOf(atype(TASK,?,?)*btype(INTEGER,?,?))));
  List_ANY_Var(Refinement(Task_r),?)==(?)
END
&
THEORY ListOfIdsX IS
  List_Of_Ids(Refinement(Task_r)) == (max_prt,schedule_p | ? | t_bpriority,t_priority,idle,running,suspended,runable,blocked,tasks,active | ? | t_create,t_delete,t_yield,t_suspend,t_resume,t_getPriority,t_getCurrent,t_getNumberOfTasks,t_delayTask,t_startScheduler,t_endScheduler,t_resumeAll,t_unblock,t_removeFromEventList,t_setPriority,t_priorityInherit,t_returnPriority | ? | seen(Machine(FreeRTOSConfig)),seen(Machine(Types)),seen(Machine(Scheduler)) | ? | Task_r);
