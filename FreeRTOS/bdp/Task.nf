...
THEORY ListOperationsX IS
  Internal_List_Operations(Machine(Task))==(t_create,t_delete,t_yield,t_suspend,t_resume,t_getPriority,t_getCurrent,t_getNumberOfTasks,t_delayTask,t_startScheduler,t_endScheduler,t_resumeAll,t_unblock,t_removeFromEventList,t_setPriority,t_suspendAll,t_priorityInherit,t_returnPriority);
  List_Operations(Machine(Task))==(t_create,t_delete,t_yield,t_suspend,t_resume,t_getPriority,t_getCurrent,t_getNumberOfTasks,t_delayTask,t_startScheduler,t_endScheduler,t_resumeAll,t_unblock,t_removeFromEventList,t_setPriority,t_suspendAll,t_priorityInherit,t_returnPriority)
END
&
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(Task),t_resumeAll)==(unblocked: FIN(TASK) & unblocked <: blocked & running: runable);
...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Machine(Task),t_returnPriority)==(btask: TASK & btask: tasks & btask: blocked\/suspended & active = TRUE & running/=idle | btask: blocked ==> blocked:=blocked-{btask} [] not(btask: blocked) ==> skip || (btask: suspended ==> suspended:=suspended-{btask} [] not(btask: suspended) ==> skip) || @n_runable.(n_runable <: tasks & n_runable = runable\/{btask} ==> (runable:=n_runable || (@(running$0).(running$0: n_runable ==> running:=running$0) [] skip))));
  ...
  Expanded_List_Substitution(Machine(Task),t_removeFromEventList)==(task: TASK & task: blocked\/suspended & active = TRUE | task: blocked ==> blocked:=blocked-{task} [] not(task: blocked) ==> skip || (task: suspended ==> suspended:=suspended-{task} [] not(task: suspended) ==> skip) || @n_runable.(n_runable <: tasks & n_runable = runable\/{task} ==> (runable:=n_runable || (@(running$0).(running$0: n_runable ==> running:=running$0) [] skip))));
  Expanded_List_Substitution(Machine(Task),t_unblock)==(task: TASK & task: blocked & active = TRUE | blocked:=blocked-{task} || @n_runable.(n_runable <: tasks & n_runable = runable\/{task} ==> (runable:=n_runable || (@(running$0).(running$0: n_runable ==> running:=running$0) [] skip))));
  Expanded_List_Substitution(Machine(Task),t_resumeAll)==(unblocked: FIN(TASK) & unblocked <: blocked & running: runable | unblocked/={} ==> (@n_runable.(n_runable <: tasks & n_runable = runable\/unblocked ==> (runable:=n_runable || (@(running$0).(running$0: n_runable ==> running:=running$0) [] skip))) || blocked:=blocked-unblocked) [] not(unblocked/={}) ==> skip);
  ...
  List_Substitution(Machine(Task),t_delete)==(tasks:=tasks-{atask} || IF atask: runable THEN ANY n_runable WHERE n_runable <: tasks & n_runable = runable-{atask} THEN runable:=n_runable || IF atask = running THEN IF active = TRUE THEN running:: n_runable END END END ELSIF atask: blocked THEN blocked:=blocked-{atask} ELSIF atask: suspended THEN suspended:=suspended-{atask} END);
  ...
  List_Substitution(Machine(Task),t_returnPriority)==(IF btask: blocked THEN blocked:=blocked-{btask} END || IF btask: suspended THEN suspended:=suspended-{btask} END || ANY n_runable WHERE n_runable <: tasks & n_runable = runable\/{btask} THEN runable:=n_runable || CHOICE running:: n_runable OR skip END END)
END
&
