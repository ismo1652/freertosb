...
THEORY ListVariablesX IS
  ...
  List_Variables(Machine(FreeRTOSBasic))==(tickMissed,tickCount,unblocked,idle,running,suspended,runable,blocked,tasks,active,first_receiving,first_sending,mutex_holder,queue_sending,queue_receiving,queue_items,mutexes_busy,mutexes,semaphores_full,semaphores_busy,semaphores,queues_msg_empty,queues_msg_full,queues_msg,queues);
  External_List_Variables(Machine(FreeRTOSBasic))==(tickMissed,tickCount,unblocked,idle,running,suspended,runable,blocked,tasks,active,first_receiving,first_sending,mutex_holder,queue_sending,queue_receiving,queue_items,mutexes_busy,mutexes,semaphores_full,semaphores_busy,semaphores,queues_msg_empty,queues_msg_full,queues_msg,queues)
END
&
...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(FreeRTOSBasic))==(tickCount: TICK & tickMissed: TICK & !que.(que: queues => queue_sending(que) <: blocked\/suspended) & !que.(que: queues => queue_receiving(que) <: blocked\/suspended) & ran(mutex_holder) <: tasks\/{TASK_NULL} & !mt.(mt: mutexes => queue_sending(mt) = {}) & (active = TRUE => idle/:ran(mutex_holder) & !mt.(mt: mutexes_busy => mutex_holder(mt): tasks) & !mt.(mt: mutexes & mt/:mutexes_busy => mutex_holder(mt)/:tasks)))
END
&
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(FreeRTOSBasic),incrementTick)==(active = TRUE & active = TRUE & TICK_INCREMENT(tickCount,1): TICK | tickCount:=TICK_INCREMENT(tickCount,1) || @n_unblocked.(n_unblocked: FIN(TASK) & n_unblocked <: blocked ==> unblocked:=n_unblocked));
  ...
  Expanded_List_Substitution(Machine(FreeRTOSBasic),beforeResumeAll)==(active = FALSE & tickCount: TICK & TICK_INCREMENT(tickCount,tickMissed): TICK | @n_unblocked.(n_unblocked: FIN(TASK) & n_unblocked <: blocked ==> unblocked:=n_unblocked));
  ...
  List_Substitution(Machine(FreeRTOSBasic),beforeResumeAll)==(t_beforeResumeAll(tickCount,TICK_INCREMENT(tickCount,tickMissed)));
  List_Substitution(Machine(FreeRTOSBasic),xTaskResumeAll)==(t_resumeAll(unblocked) || q_resumeAll(unblocked) || tickCount:=TICK_INCREMENT(tickCount,tickMissed));
  ...
  List_Substitution(Machine(FreeRTOSBasic),sendItem)==(IF pxQueue/:queues_msg_full THEN q_sendItem(pxQueue,pvItemToQueue,copy_position) || IF queue_receiving(pxQueue)/={} THEN t_removeFromEventList(first_receiving(pxQueue)) END || return:=pdPASS ELSE CHOICE t_delayTask(xTicksToWait,running) || q_insertTaskWaitingToSend(pxQueue,running) || return:=pdPASS OR t_suspend(running) || q_insertTaskWaitingToSend(pxQueue,running) || return:=pdPASS OR return:=pdFAIL END END);
  ...
  List_Substitution(Machine(FreeRTOSBasic),incrementTick)==(tickCount:=TICK_INCREMENT(tickCount,1) || t_incrementTick(TICK_INCREMENT(tickCount,1)));
