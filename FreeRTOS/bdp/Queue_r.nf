...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Refinement(Queue_r))==(queue_quant,queue_items_r,queue_size,first_receiving,first_sending,mutex_holder,queue_sending,queue_receiving,mutexes_busy,mutexes,semaphores_busy,semaphores,queues_msg_full,queues_msg_empty,queues_msg,queues);
  List_Variables(Refinement(Queue_r))==(queue_quant,queue_items_r,queue_size,first_receiving,first_sending,mutex_holder,queue_sending,queue_receiving,mutexes_busy,mutexes,semaphores_busy,semaphores,queues_msg_full,queues_msg_empty,queues_msg,queues);
  External_List_Variables(Refinement(Queue_r))==(queue_quant,queue_items_r,queue_size,first_receiving,first_sending,mutex_holder,queue_sending,queue_receiving,mutexes_busy,mutexes,semaphores_busy,semaphores,queues_msg_full,queues_msg_empty,queues_msg,queues)
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Refinement(Queue_r))==(queues,queues_msg,queue_receiving,queue_sending,semaphores,semaphores_busy,mutexes,mutexes_busy,mutex_holder,queue_size,queue_items_r,queue_quant,queues_msg_empty,queues_msg_full,first_sending,first_receiving:={},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{});
  Context_List_Initialisation(Refinement(Queue_r))==(skip);
  List_Initialisation(Refinement(Queue_r))==(queues:={} || queues_msg:={} || queue_receiving:={} || queue_sending:={} || semaphores:={} || semaphores_busy:={} || mutexes:={} || mutexes_busy:={} || mutex_holder:={} || queue_size:={} || queue_items_r:={} || queue_quant:={} || queues_msg_empty:={} || queues_msg_full:={} || first_sending:={} || first_receiving:={})
END
&
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Refinement(Queue_r),q_endScheduler)==(btrue | queues,queues_msg,queue_receiving,first_receiving,queue_sending,first_sending,semaphores,semaphores_busy,mutexes,mutexes_busy,mutex_holder,queue_quant,queue_size,queues_msg_empty,queues_msg_full,queue_items_r:={},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{});
  ...
  List_Substitution(Refinement(Queue_r),q_takeSemaphore)==(queue_quant(semaphore):=queue_quant(semaphore)-1 || IF queue_quant(semaphore)-1 = 0 THEN semaphores_busy:=semaphores_busy\/{semaphore} END || ANY n_sending,n_first WHERE n_sending: POW(TASK) & n_sending = queue_sending(semaphore)-{first_sending(semaphore)} & n_first: TASK & n_first: n_sending THEN queue_sending(semaphore):=n_sending || first_sending(semaphore):=n_first END);
  List_Substitution(Refinement(Queue_r),q_giveSemaphore)==(IF queue_quant(semaphore)<queue_size(semaphore) THEN queue_quant(semaphore):=queue_quant(semaphore)+1 || IF queue_quant(semaphore) = 0 THEN semaphores_busy:=semaphores_busy-{semaphore} END || ANY n_receiving,n_first WHERE n_receiving: POW(TASK) & n_receiving = queue_receiving(semaphore)-{first_receiving(semaphore)} & n_first: TASK & n_first: n_receiving THEN queue_receiving(semaphore):=n_receiving || first_receiving(semaphore):=n_first END END);
  ...
  List_Substitution(Refinement(Queue_r),q_endScheduler)==(queues:={} || queues_msg:={} || queue_receiving:={} || first_receiving:={} || queue_sending:={} || first_sending:={} || semaphores:={} || semaphores_busy:={} || mutexes:={} || mutexes_busy:={} || mutex_holder:={} || queue_quant:={} || queue_size:={} || queues_msg_empty:={} || queues_msg_full:={} || queue_items_r:={});
...
THEORY VariablesEnvX IS
  Variables(Refinement(Queue_r)) == (Type(queue_quant) == Mvl(SetOf(atype(QUEUE,?,?)*btype(INTEGER,?,?)));Type(queue_items_r) == Mvl(SetOf(atype(QUEUE,?,?)*SetOf(btype(INTEGER,?,?)*atype(ITEM,?,?))));Type(queue_size) == Mvl(SetOf(atype(QUEUE,?,?)*btype(INTEGER,?,?)));Type(first_receiving) == Mvl(SetOf(atype(QUEUE,?,?)*atype(TASK,?,?)));Type(first_sending) == Mvl(SetOf(atype(QUEUE,?,?)*atype(TASK,?,?)));Type(mutex_holder) == Mvl(SetOf(atype(QUEUE,?,?)*atype(TASK,?,?)));Type(queue_sending) == Mvl(SetOf(atype(QUEUE,?,?)*SetOf(atype(TASK,?,?))));Type(queue_receiving) == Mvl(SetOf(atype(QUEUE,?,?)*SetOf(atype(TASK,?,?))));Type(mutexes_busy) == Mvl(SetOf(atype(QUEUE,?,?)));Type(mutexes) == Mvl(SetOf(atype(QUEUE,?,?)));Type(semaphores_busy) == Mvl(SetOf(atype(QUEUE,?,?)));Type(semaphores) == Mvl(SetOf(atype(QUEUE,?,?)));Type(queues_msg_full) == Mvl(SetOf(atype(QUEUE,?,?)));Type(queues_msg_empty) == Mvl(SetOf(atype(QUEUE,?,?)));Type(queues_msg) == Mvl(SetOf(atype(QUEUE,?,?)));Type(queues) == Mvl(SetOf(atype(QUEUE,?,?))))
END
&
