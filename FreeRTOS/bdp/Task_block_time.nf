...
THEORY ListInvariantX IS
  ...
  List_Invariant(Refinement(Task_block_time))==(t_tblock: TASK +-> TICK & dom(t_tblock) = blocked & !task.(task: unblocked => t_tblock(task) = tickCount))
END
&
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Refinement(Task_block_time),t_incrementTick)==(btrue | active = TRUE ==> (tickCount:=TICK_INCREMENT(tickCount,1) || @n_unblocked.(n_unblocked: FIN(TASK) & n_unblocked = {task | task: blocked & t_tblock(task) = TICK_INCREMENT(tickCount,1)} ==> unblocked:=n_unblocked)) [] not(active = TRUE) ==> (tickMissed:=TICK_INCREMENT(tickMissed,1) || @n_unblocked.(n_unblocked: FIN(TASK) & n_unblocked = {task | task: blocked & t_tblock(task) = tickCount} ==> unblocked:=n_unblocked)));
  Expanded_List_Substitution(Refinement(Task_block_time),t_beforeResumeAll)==(active = FALSE | tickCount,tickMissed:=TICK_INCREMENT(tickCount,tickMissed),0 || @n_unblocked.(n_unblocked: FIN(TASK) & n_unblocked = {task | task: blocked & t_tblock(task) = TICK_INCREMENT(tickCount,tickMissed)} ==> unblocked:=n_unblocked));
  ...
  List_Substitution(Refinement(Task_block_time),t_beforeResumeAll)==(tickCount:=TICK_INCREMENT(tickCount,tickMissed) || tickMissed:=0 || ANY n_unblocked WHERE n_unblocked: FIN(TASK) & n_unblocked = {task | task: blocked & t_tblock(task) = TICK_INCREMENT(tickCount,tickMissed)} THEN unblocked:=n_unblocked END);
  List_Substitution(Refinement(Task_block_time),t_incrementTick)==(IF active = TRUE THEN tickCount:=TICK_INCREMENT(tickCount,1) || ANY n_unblocked WHERE n_unblocked: FIN(TASK) & n_unblocked = {task | task: blocked & t_tblock(task) = TICK_INCREMENT(tickCount,1)} THEN unblocked:=n_unblocked END ELSE tickMissed:=TICK_INCREMENT(tickMissed,1) || ANY n_unblocked WHERE n_unblocked: FIN(TASK) & n_unblocked = {task | task: blocked & t_tblock(task) = tickCount} THEN unblocked:=n_unblocked END END)
END
&
