...
Semáforos são estruturas de sicronização entre tarefas. Com isso, eles são
utilizados para coordenar o uso de um recurso compartilhado por uma ou mais
tarefas. Um exemplo de como é utilizado um semáforo no FreeRTOS pode ser visto na
figura~\ref{fig:app_semaforo}. Nela primeiramente, através do método
\textit{xSemaphoreCreateMutex}, o semáforo \textit{xSemaphore} é criado. Em
seguida, a tarefas \textit{semaphoreTask} é criada para demonstrar a utilização
do semáforo.

Nas tarefa \textit{semaphoreTask}, parte do seu código é protegido para ser
executado somente quando o semáforo for retido. Assim para solicitar o semáforo
essa tarefa utiliza-se do método \textit{xSemaphoreTake}, que tem como
parâmentros de entrada o semâforo que deve ser solicitado e tempo que a tarefa
pode ficar bloqueada aguardando pelo semáforo, no caso no semáforo estar
oculpado. Assim, para informar se o semáforo foi obitido ou não a função
``falso'' se o semáforo estiver oculpado e verdadeiro se este estive disponível.
Com isso, caso o semáforo estajá desponível e for retido pela tarefa a para na
tarefa protegida pelo semáforo é executada e o semáforo é liberado em seguida.
