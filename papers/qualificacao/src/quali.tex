...
As substituições simples e multiplas permitem somente um mode de especificação
onde as atribuições são feitas de maneira uniforme sem levar em consideração os
estados iniciais da operação. Entretanto linguagens de programação provem um
tipo condicional de atribuição na qual é permitido caminhos diferente de acordo
com o valor inicial de variáveis, sendo o mecanismo utilizado para que tal
escolhar seja realizada é a construção if then else. 

Assim como nas liguangens de programação, a notação de máquina abstrata também
permite a construção de estrutura de condição, através da substituição
condicional, que possui a seguinte forma:
$$\IF \mathit{E} \THEN \mathit{S} \ELSE \mathit{T}$$

Nela $\mathit{S}$ e $\mathit{T}$ são substituições que são executadas ou não,
dependedo a avaliação da exepressão booleana $\mathit{E}$. Assim caso
$\mathit{E}$ sejá afirmativo a substituição $\mathit{S}$ é realizada e caso
negativo a substituição $\mathit{T}$ é realizada. Com isso a substituição
condicional pode ser interpretada da seguinte forma:

$$[\IF \mathit{E} \THEN \mathit{S} \ELSE \mathit{T}] \mathit{P} = \mathit{E}
\Longrightarrow [\mathit{S}] P \land  \not\mathit{E}
\Longrightarrow [\mathit{T}] P$$.
