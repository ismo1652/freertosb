...
% Espírito Santo} \end{folhadeaprovacao} \begin{resumo} Escreva aqui o texto do
...
conseqüências catastróficas. É, devido a isso, a grande necessidade do
...
eles está o projeto do ``Software Verificado''. O Software Verificado é um
conjunto de teorias, ferramentas e experiências usadas para verificação e
validação de softwares. Ao final desse projeto, pretende-se criar um verificador
de softwares que garanta o comportamento fidedigno do sistema analisado. Outra
atividade associada a esse projeto é a criação de estudos de caso a serem
utilizados pelas abordagens proposta. Entre tais estudos de caso, um dos mais
interessantes e proveitosos é sem dúvida a especificação formal do sistema
operacional de tempo real FreeRTOS \cite{Hoare05}\cite{Jim09}.
...
especificação desse sistema seria uma grandes contribuições, principalmente para
...
trabalho já realizada é demonstrada no capitulo~\ref{cap:primeiros_passos} e a
...
\begin{itemize}
  ...
  fila;
  \item Acrescentar os demais elementos do FreeRTOS à especificação;e
...
examinada. Assim, os principais requisitos, funcionalidade e características e do
...
módulos incrementais, os quais são compostos pela funcionalidades e
...
organizados de acordo com a figura~\ref{fig:arquitetura}. Os módulo mais abaixo
...
Ao total, essa especificação inicial possui 1974 linhas, as quais geraram 538
obrigações de prova (seção~\ref{subsec:obrigacao_de_prova}). Entre essas
obrigações de prova, somente 49 necessitaram da interação humana para sua
...
entidades do FreeRTOS; e refinar a modelagem para abrangir os requisitos do
...
Ao final da especificação, pretende-se atingir uma modelagem concrentra do
...
disponibilizar diversas visões do sistema e as partes de domínio e plaforma podem
ser parametrizáveis, podendo assim serem reutilizadas em várias aplicações.
...
modelo CSP, as atividades implementadas no sistema são abstraidas para um ponto
de vista mais prático e abstrato, permitindo assim uma melhor análise do sistema.

No trabalho de Solá, o formalismo utilizado, Event B, é muito parececido com o
método B. Ele também é formado por estados, que devem ser mantidos, e operações,
denomidas de eventos, responsáveis por alterar esses estados. Assim, na
especificação do L4, as abstrações de hardware do sistema, como \textit{threads}
e comunicação IPC, foram especificadas através de estados e as chamadas da API
do sistema através de eventos.
...
operacional convencional, sendo essas: a camada das primitivas de
 ...
 proporciona uma modelagem incremental de um SO, na quais os elementos mais
...
O FreeRTOS é um Sistema Operacional de Tempo Real-SOTR enxuto, simples e de fácil
uso. O seu código fonte, feito em \textit{C} e com partes em \textit{assembly},
é aberto e possui um pouco mais de 2.200 linhas de código, que são essencialmente
distribuídas em quatro arquivos: \textit{task.c}, \textit{queue.c},
\textit{croutine.c} e \textit{list.c}. Uma outra característica marcante desse
sistema está na sua portabilidade, sendo o mesmo oficialmente portável para 17
arquiteturas mono-processadores diferentes, entre elas a PIC, ARM e Zilog Z80, as
quais são amplamente difundidas em produtos comerciais através de sistemas
computacionais embutidos \cite{Barry09}.
...
aplicação e o hardware, que tem o papel esconder dos desenvolvedores de
...
aos desenvolvedor serviços como gerenciamento de tarefa, comunicação e
...
configuração é feita por uma biblioteca configuração, que, através de atributos,
armazena as definições de configuração do usuário. Com isso, as aplicações
...
\begin{description}
  ...
  ambiente de execução quando suas atividades são suspensas.
...
a situação da mesma. O FreeRTOS disponibiliza quatro tipos de estados diferentes
...
\begin{description}
  ...
  evento para continuar a sua execução.
...
No FreeRTOS, em um determinado instante, uma tarefa pode assumir somente um
estado. Assim, as alterações de estado de uma tarefa funcionam como demonstra o
diagrama da figura~\ref{fig:task_states}. Nela, uma tarefa com o estado EM
EXECUÇÃO pode assumir os estados PRONTA, BLOQUEADA ou SUSPENSA. Uma tarefa com o
estado PRONTA pode ser suspensa ou entrar em execução, e as tarefas com o estado
BLOQUEADA ou SUSPENSA só podem ir para o estado PRONTA.
...
Por fim, vale enfatizar que, por tratar-se de um SOTR para arquiteturas
...
estado PRONTA pode assumir o processador e entrar no estado EM EXECUÇÃO. Com
isso, para decidir qual tarefa entrará em execução, o FreeRTOS possui um
mecanismo denominado escalonador, o qual será detalhado na
...
uma área da memória é reservada à ela, em seguinda, quando essa tarefa é excluída
do sistema, a memória destinada ela continua ocupada, sendo esta liberada somente
quando a tarefa ociosa entra em execução. A tarefa ociosa deve possuir prioridade
menor que as demais tarefas do sistema e, por isso, ela só é executada quando
...
situação da mesma. Para o FreeRTOS, o suporte às Co-rotinas é opcional e pré
configurável antes da execução, em tempo de compilação \cite{Barry09}.
...
utilização desse elemento deve ser feita de forma cuidadosa, pois uma informação
armazenada por uma co-rotinas pode ser alterada por outra. Por exemplo a
...
Assim, quando a co-rotina \textit{A} for ler novamento o valor armazenado, este
...
\begin{itemize}
  ...
  um determinado tempo. Nessa funcinalidade, para calcular o tempo que a tarefa
  ...
  interrupção no momento da chamada à funcionadade, o instante que a tarefa foi
  desbloqueada não ira mudar. Com isso, esse método torna-se recomendável para a
...
\begin{itemize}
  ...
  uma referência para atual tarefa em execução.
  ...
  'tag', que será utilizada principalmente pelas funcionalidades de
  rastro do sistema, a uma tarefa. Entranto, é possível usar essa 'tag' para
  associar uma função gancho a uma tarefa. Essa função gancho será executada através da
  chamada à funcionalidade \textit{TaskCallApplicationTaskHook}, informando
  a tarefa associada;
...
responsáveis por criar e gerenciar o elemento co-rotina. A lista completas das
funcionalidades presentes nessa bibliteca pode ser vista a seguir:
...
receber uma mensagem de uma fila através de uma co-rotina responsável por tratar
uma interrupção.
...
disponibilizar em uma tela as teclas digitadas pelo usuário. Essa comunicação
necessita ser estruturada e sem interrupções, para isso deve ser feita através
de mecanismos específicos.

A maioria dos sistemas operacionais oferece vários tipos de comunicação entre as
tarefas. Geralmente esse tipos são: Tarefas trocando informações entre si;
Tarefas utilizando, de forma sicronizada, o mesmo recurso; Tarefas dependentes
...
funciona essa comunicação, cada um desses mecanismo será detalhado a seguir.

\subsection{Fila de Mensagens}
\label{sec:fila}

Filas de mensagens são estruturas primitivas de comunicação entre tarefas. Elas
funcionam como um túnel, atravé do qual tarefas enviam e recebem mensagem
...
\begin{itemize}
  ...
  \item Uma variável que indica o tamanho das mensagem da fila; 
...
A retirada de uma tarefa listas de espera de uma fila é feita levando em
...
com contador existe um número fixo de tarefa (maior ou igual à um) que podem reter o
semáforo. 
...
No FreeRTOS os semáforos são implementados como uma fila de mensagens que informa
o estado do semáforo através da situação da fila. Assim, quando a fila estiver
...
semáforo binário, é criada uma fila de capacidade um e, para representar uma
...
responsável pela criação e utilização do elemento de fila de mensagens. Nele,
...
do sistema. Assim, com o intuito de ajudar o desenvolvedor a criar suas
primeiras aplicações, o FreeRTOS disponibilizou, junto com seu código fonte, aplicações
...
FreeRTOS, demonstrando seus principais elementos, funcionalidades e
características. Com isso, para um melhor entedimento das explicações
apresentadas neste capítulo, será demonstrada, nas seções seguintes, de forma
didática, a utilização dos principais elementos aqui discutidos, tarefa,
co-rotinas, fila de mensagens e semáforos.

\subsection{Utilização do elemento tarefa}
...
a tarefa,  o escalonador do sistema deve ser iniciado e com ele a aplicação .
Essa operação é feita pela a funcionalidade \textit{vTaskStartScheduler()},
localizada no final do código.
...
mensagem, o local em que a mensagem o endereço da mensagem irá ser armazenado, e
o tempo máximo que a tarefa pode ficar esperando pela fila. Como retorno, essa
funcionalidade informa se a mensagem foi retirada da fila com sucesso ou não,
sendo com isso utilizada como guarda para o código que deve ser executado após a
...
o usuário, entre os dois mecanismos está apenas no método de criação do elemento,
...
atividades da aplicação. Essas ações são colocadas dentro da rotinas executadas
...
Para isso ela interrompe a sua execução durante um tempo de dez ticks, utilizando
a funcionalidade \textit{crDELAY}, que tem como argumentos o gancho da co-rotina
e o tempo de bloqueio da co-rotina. Após isso, a função de controle do LED
\textit{vParTestToggleLED} é chamada. Observa-se nesse exemplo a utilização dos
parâmetros da rotina como argumentos para a funcionalidade \textit{crDELAY}.

Com a criação da rotina utilizada pela co-rotina, resta apenas criar a co-rotina
no sistema. Esse trabalho é feita através da funcinalidade
...
Por fim, devido à politica de escolonamento coopertivo das co-rotinas, ao final
...
equipamentos médicos e empresas que movimentam uma grandes quantidade monetárias,
como os bancos\cite{Jim09}.
...
Segundo a Honeywell em \cite{honeywell03}, uma empresa que desenvolve sistemas
para aeronaves, a utilização de métodos formais no processo de desenvolvimento
provê várias vantagens, entre eles estão:
\begin{itemize}
...
programação imperativas e seqüenciais. Para isso, sucessívos refinamentos devem
...
as dependências entre os vários módulos, que geralmente constituem uma
especificação. Devido as suas vastas funcionalidades e popularidade o AtelierB
...
a natureza e o nome do módulo, seguido opcionalmente por um ou mais parâmetros de
máquina, os quais são separados por vírgula e limitados por parenteses;
...
estados são definidos através das variáveis e dos seus invariantes. As variáveis
são diferentes elementos que compõem o estado do sistema. O invariante são
expressões lógicas que determinam os valores as variáveis podem assumir. Assim,
uma especificação só pode definir o correto funcionamento da máquina, quando ele
encontra-se em um estado válido, nada sendo especificado para os demais casos.

O estado de uma máquina é especifico, por meio de lógica dos predicados, teoria
dos conjuntos e aritimética dos inteiros, permitindo com isso uma análise
estática da máquina através da expressões lógicas geradas apartir de sua
...
entrada e os parâmetros de saída argumentos opcionais. O exemplo de uma operação
com parâmetros de entrada e saída pode ser visto na figura~\ref{fig:query_task}.
...
abstrata possuí um conjunto de atribuições abstratas, denominadas substituições,
...
$\mathit{S}$ a $\mathit{P}$. A seguir são demonstrados algumas das principais
...
Onde tem-se que no predicado $\mathit{P}$ a variável
...
predicado $\mathit{P}$, as variáveis $\mathit{x}$ e $\mathit{y}$ são
substituidas por $\mathit{E}$ e $\mathit{F}$, respectivamente. Por exemplo,
...
lógica é avaliada e, em seguida o caminho a estrutura deve seguir, e qual
atribuições devem ser realizada, é escolhido de acordo com o resultado da
...
substituições determinadas de substituições não determinísticas.

Substituições não determinísticas são substituições que introduzem escolhas
aleatórias no corpo da operaçõe, levando a operação a um conjunto de estados
finais diferentes a cada execução da operação. Em uma substituição não
...
conjunto de estados finais possíveis, que podem ser alcançados a cada execução
da substituição.
...
em maior detalhes cada uma dessas obrigações de prova e como elas são geradas.

\subsection{Consistência do Invariante}

Nessa obrigação de prova é analisado se o invariante da máquina possui pelo menos
uma combinação que todas variáveis tem valores válidos, ou seja, a máquina
possui pelo menos um estado válido. Essa obrigação de prova é definida da
seguinte maneira :
...
descritas de forma mais concreta, o que é possível através do refinamento.
Além disso o refinamento pode ser usado para construir um modelo abstrato de
forma incremental com da adição sucessivas de formalizaçoes dos
requisitos\ref{}.
...
modelagem abstrata até um nível algoritmo denominada de implementação. Entre
esses níveis de abstração existem modelos intermediários chamados de
refinamentos, que combina especificações de abstratas com detalhes de
implementação.
...
Como ocorreu na seção da máquina abstrata \ref{sec:maquina_abstrata}, para um 
...
uma sequência de tarefas, estrutura mais concreta que um conjunto. A relação de
...
saída. Entretanto, nas operações de um refinamento, não é necessário a
declaração da pré condição ($\PRE$), uma vez que essa foi definida em um nível mais abstrato
...
A analise estática que confere se um refinamento é consistente com o nível
...
seguir é demonstrada como é realizada cada uma dessas obrigações de prova.
...
um conjunto de diferentes estados. Assim, em um refinamento, é necessário que
cada execução de $\mathit{T1}$ possua uma execução correspondente em $\mathit{T}$. Em
...
A \textit{relação de refinamento} $\mathit{J}$ é um predicado entre variáveis
abstratas e variáveis de refinamento. Com isso, $\mathit{T}$ deve possuir pelo
menos uma transição que satisfaça esse predicado, ou seja, nem todas as
transições de $\mathit{T}$ levará $\mathit{J}$ a falsidade. Essa afirmativa pode
ser traduzida na expressação abaixo:
...
Geralmente, uma operação é definida como $\PRE \mathit{P} \THEN \mathit{S}
\END$, sendo o seu refinamento $\PRE \mathit{P1} \THEN \mathit{S1} \END$, onde na
...
\mathit{P}$. As demais verificações são similiares à obrigações de prova das
...
\chapter{Primeiros passos da Modelagem}\label{cap:primeiros_passos}

Toda longa caminhada começa com os primeiros passo. Como primeiros passos desse
...
planejamento dessa modelagem, levou-se em consideração principalmente as
...
refinamentos horizontais ou extensão da especificação, o que proporciona uma
...
baseadas no elemento fila de mensagens, sendo este um elemento necessário em
qualquer modelagem inicial. Assim, nesse primeiro passo, devido às suas
utilidades, as estruturas escolhidas para serem formalizadas, junto com algumas
de suas funcionalidades, foram: tarefa, fila de mensagens e
escalonador\footnote{As funcionalidades e propriedades do escalonador serão
tratadas dentro do elemento tarefa }.


\section{Tarefa}\label{sec:model_task}
Nesse ponta pé inicial, para tornar-se possível a criação da modelagem funcional,
muitas propriedades do elemento tarefa foram abstraídas. Resumidamente, apenas a
...
A especificação do elemento tarefa foi planejada de forma incremental.
Previamente, foram criados os estados necessários para modelar tal elemento no
nível de abstração sugerido. Após isso, algumas das funcionalidades do sistema,
relacionadas a esse elemento, foram formalizadas a cada etapa de criação do
...
Para o elemento fila de mensagens, a principal propriedade especificada foi a
quantidade de mensagens que esta pode armazenar. Como ocorreu com o elemento
Tarefa, a formalização dessa entidade, junto com suas funcionalidades, foi
distribuida entre as várias etapas de desenvolvimento da modelagem inicial. Com
...
funcionalidade relacionadas à fila de mensagens tratadas nessa especificação
...
A modelagem discultida nas seções anteriores foi construída e verifica utilizando
a ferramenta AtelierB 4.0\cite{clearsy09}. Nessa modelagem, foram especificados
três elementos básicos do sistema: tarefa, fila de mensagens e escalonador. Tal
...
\begin{description}
  ...
  responsáveis por formalizar o elemento tarefa.
  \item{\textbf{Módulo Queue:}} Esse módulo é similar ao módulo Task, só que
  nele o elemento especificado é a fila de mensagens.
  ...
  especificadas as funcionalidades das APIs do FreeRTOS.
\end{description}
...
detalhes como o desenvolvimento dessa especificação inicial.
...
mapeamente, sequências e conjuntos. A abordagem escolhida nesse trabalho inicial
foi a representação através de conjuntos, o que facilita a verificação da
especificação pelo provador de teoremas. Assim, a modelagem do elemento tarefa e
suas propriedades foram feitas de acordo figura~\ref{fig:task_state}. Nela, a
variável $\mathit{active}$ informa quando o sistema está ativado. A variável
...
$\mathit{suspended}$ armazenam tarefas nos estados rodando, pronto, bloqueado e
suspenso, respectivamente. Por fim, a variável $\mathit{idle}$ representa a
tarefa ociosa.
...
servirão como base para a modelagem das funcionalidades do FreeRTOS relacionadas
ao elemento Tarefa. Ao total, foram criadas doze operações elementares, das quais
...
esse parâmetro só será utilizado no refinamento da operação. Por fim, a operação
...
de mensagens. Essa especificação é feita através da máquia $\mathit{Queue}$.
...
mensagem na fila(fila vazia) e um conjunto de tarefas bloqueadas aguardando para
enviar uma mensagem para a fila(fila cheia).
...
Prosseguindo na especificação, percebeu-se que as funcionalidades relacionadas ao
elemento fila de mensagens, possuem um comportamento muito parecido. Além disso,
para a especificação dessas funcionalidades é necessário ter-se uma ligação com
os elementos tarefas e fila de mensagens, pois uma fila é capaz de bloquear e
desbloquear tarefas. 

Nesse contexto, para especificar os comportamentos comuns das funcionalidades da
fila de mensagens e fazer a ligação dessas funcionalidades com as do elemento
tarefa, foram criadas, na máquina $\mathit{FreeRTOSBasic}$, as seguinte funções
...
requisito. Um vez inicializado o escalonador, a tarefa em execução deve ter
...
capítulo~\ref{sec:primeiros_passos}, parte dessa especificação já foi realizada.
Portanto, para atingir o objetivo proposto, é necessário formalizar os elementos,
funcionalidades e requisitos do sistema ainda não tratados.

Detalhando a parte do sistema não especificada, tem-se inicialmente algumas
funcionalidades das entidades tarefas e fila de mensagens, especificadas de
formar abstrata, de modo que várias de suas características não poderam ser
tratadas. Portanto, uma parte desse trabalho a ser realizada será a especificação
das demais funcionalidades relacionadas a essas entidades. Uma explicação mais
detalhada das funcionalidades a serem modeladas será vista nas seções
\ref{sec:tarefa_proposta} e \ref{sec:fila_proposta}.

Além da especificação das funcionalidades anteriores, é necessário também
adicionar à modelagem, os elementos Semáforo, Mutex e Co-Rotinas. Essa adição
ocorrerá de forma incremental e somente algumas características desses elementos
serão tratados inicialmente. Por fim, com o intuito de incrementar a modelagem
com características ainda não tratadas na especificação das entidades, um
refinamento da modelagem será desenvolvido. Maiores detalhes sobre a
especificação desses elementos e o refinamento da especificação serão vistos nas
seções \ref{sec:semaforo_proposta}, \ref{sec:mutex_proposta},
\ref{sec:corotina_proposta} e \ref{sec:requisitos_proposta}.

\section{Entidade Tarefa}
\label{sec:tarefa_proposta}

A entidade tarefa já foi parcialmente formalizada pela modelagem inicial. Foram,
especificadas, de forma abstrata, algumas das seguintes funcionalidades
relacionadas a esse elemento, criação de
tarefa(seção~\ref{sec:criacao_tarefa_api}), controle de
tarefas(seção~\ref{sec:controle_tarefa_api}), utilitários de
tarefa(seção~\ref{sec:utilitarios_tarefa_api}) e controle de
escalonador(seção~\ref{sec:controle_escalonador_api}).

Entretanto, muitas funcionalidades da API de tarefa foram deixadas de fora da
especificação inicial. Em suma, tem-se que:
\begin{itemize}
  \item Na parte de criação de tarefa, todas as suas funcionalidades foram
  formalizadas de forma abstrata;
  \item No controle de tarefas, apenas a funcionalidade
  \textit{xTaskResumeFromISR()}, utilizada pelo tratamento de interrupções para
  retornar uma tarefa suspensa, não foi especificada;
  \item Na utilitários de tarefa, foram deixadas de lado as funcionalidades
  \textit{uxTaskGetStackHighWaterMark()}, que retorna a quantidade de espaços
  vazios na pilha de uma tarefa, \textit{vTaskSetApplicationTag()}, que associa
  uma função gancho a uma tarefa, e \textit{xTaskCallApplicationTaskHook()}, que
  realiza a chamada a uma função gancho de uma determinada tarefa; e 
  \item No controle de escalonador faltam especificar as funcionalidades
  \textit{taskYIELD()}, funcionalidade que força a troca de contexto,
  \textit{taskENTER\_CRITICAL()}, funcionalidade que indica o início
  de uma região crítica, \textit{taskEXIT\_CRITICAL()}, funcionalidade
  que indica o final de uma região crítica, \textit{taskDISABLE\_INTERRUPTS()},
  funcionalidade que desabilita o uso de interrupções, e
  \textit{taskENABLE\_INTERRUPTS()}, funcionalidade que abilita o uso de
  interrupções.
\end{itemize} 

Vale ressaltar que existem funcionalidades, relacionadas à entidade tarefa, não
tratadas no modelo inicial que não serão listadas como proposta desse trabalho.
Isso, devido ao fato de que tais funcionalidades são utilizadas apenas como
funções de rastreamento do sistema, coletando informações do funcionamento do
sistema sem alterar o seu estado, sendo desnecessárias na implementação dos
...
somente quatro funcionalidades relacionadas a esse elemento foram formalizadas,
restando ainda cinco funcionalidades para completar a API referente a fila de
mensagens, sendo elas:

\begin{itemize}
  \item  \textit{uxQueueMessagesWaiting()}, fucinalidade que retorna o número de
  mensagens armazenadas em uma fila;
...
Assim como ocorreu com o elemento tarefa, também existem funcionalidades sobre as
filas de mensagens que não serão tratadas como proposta desse trabalho. Essa
decisão foi tomada devido a essas funcionalidades estarem relacionadas as
funções de rastro do sistema.
...
inicialmente, é necessário que novas entidades sejam acrescentadas ao modelo.
Esse acréscimo, assim como ocorreu com as entidade já tratadas, será feito de
forma incrementa, tornando a especificação mais simples. Sendo algumas
características desses elementeos adicionadas ao modelo através da técnica de
refinamento do método B.

A primeira entidade a ser acrescentado ao modelo será o semáforo. Inicialmente
será adicionado somente o semáforo binário (seção~\ref{sec:semaforo}). Após isso,
as alterações necessárias para manejar o semáforo com contado
(seção~\ref{sec:semaforo}) serão editadas. A especificação dessa entidade
ocorrerá através de um módulo que irá manusear os estados necessários à
especificação de suas características. Devido a isso, esse módulo servirá como
base para a especificação das funcionalidades relacionadas a essa entidade.

As características da entidade semáforo inicialmentes tratadas com a sua
especificação são: estado de um semáforo (disponível ou indisponível), conjunto
de tarefas bloqueadas à espera do semáforo e a tarefa que possui o semáforo. Ao
final, as seguintes funcinalidades irão enriquecer o modelo:
...
A criação do elemento mutex(seção~\ref{sec:mutex}) ocorrerá após a especificação
do semáforo. Isso é devido ao mutex ser considerado um tipo especial de semáforo,
no qual o mecanismo de herança de prioridade é implementado. Assim, para
adicionar o elemento mutex na modelagem do FreeRTOS será necessário apenas editar
o módulo do semáforo para tratar do mutex, junto com o seu mecanismo de herança
de prioridade.

As características do elemento mutex que serão tratadas inicialmente são: o
estado de um mutex (disponível ou indisponível), o conjunto de tarefas aguardando
o mutex, a tarefa que retém pelo mutex e a herança de prioridade. Ao final, a
funcionalidade $\mathit{xSemaphoreCreateMutex()}$, reponsável por criar um mutex
no sistema, será acrescentada à modelagem do FreeRTOS.
...
(seção~\ref{sec:corotinas}). Para formalizar essa entidade, será preciso
criar-se um novo módulo com os estados necessários para a especificação das
funcionalidades e características propostas para esta entidade. Além disso,
devido a co-rotina também ser uma unidade de execução do sistema, ela é
controlada pelo escalonador, o qual deverá ter suas funcionalidades alteradas
para suportar o elemento em questão.
...
Com a modelagem de das entidades do sistema, uma parte dos requisitos do sistema
poderão ser tratados. Entretanto, como essa modelagem será realizada
inicialmente de forma abstrata, vários requisitos não poderão ser especificados
logo de inicio. Assim, para que os demais requisitos sejam formalizados, é
necessário adicionar as características abstraídas das entidades na modelagem
inicial. Essa adição pode ocorrer de duas formas, reformulando o modelo criado
ou refinando-o.

A seguir, tem-se a lista de todos os requisitos de sistemas que não serão
tratados na especificados das entidades do sistema. uma explicação mais
detalhada desses requisitos será mostrada nos parágrafos seguintes:
...
exigência, serão necessárias alterações no módulo de tratamento das tarefas,
incrementando o mesmo com estados e operações que sejam capazes de controlar o
tempo de execução de entre as tarefas com estado PRONTA e com prioridades iguais
a tarefa em execução. Esse requisito é importante porque evita que tarefas de
maiores prioridades nunca sejam escalonadas. Após isso, a funcionalidade
\textit{incrementTick} deve ser alterada para compartilhar o tempo de execução
entre tarefas de mesma prioridade.

A característica de tamanho de uma fila de mensagens define uma capacidade máxima
para uma fila de mensagens. Essa especificação ocorrerá principalmente com o
refinamento do módulo de fila de mensagens, adicionando a ele estados e operações
capazes de controlar o tamanho de uma fila. Ao final, as funcionalidades que
estão relacionadas a essa característica devem ser refinadas para tratar tal
exigência.

Outra particularidade do sistema não tratada é a característica de uma tarefa
possuir um tempo máixmo de bloqueio\footnote{Na modelagem inicial do elemento
tarefa foi tratada apenas a característica de uma tarefa possuir o estado bloqueada e
não o tempo que essa tarefa deve permanecer nesse estado}. Para tratar essa
característica, no módulo de tarefas, será adicionado um estado para controle do
tempo de bloqueio de uma tarefa. Com isso, as funcionalidades relacionadas a
essa característica devem ser alteradas para tratar tal particularidade.

O estouro do tempo de execução acontece devido às limitações da máquina utilizada
pelo sistema. Ele ocorre quando o tempo de execução do sistema ultrapassa o
limite máximo do tipo que o representa. Para tratar esse acontecimento, o sistema
basicamente maneja as tarefas bloqueadas para que essas possam atualizar o seu
tempo de desbloqueio a cada estouro. Com isso, a especificação dessa exigência
será feita através de alterações no módulo de tarefa e conseguentemente nas
funcionalidade relacionadas as tarefas bloqueadas, além do refinamento do modulo
que preocupa-se com o tempo do escalonador.

Ordenar os itens em um fila de mensagens é implementar na mesma uma estrutura de
fila, na qual cada item tem sua posição. Essa característica será adicionada ao
modelo através do refinamento do módulo de fila, proporcionando uma estrutura
ordenada para os itens de uma fila. Após isso, as funcionalidades que cuidam
dessa característica também serão refinadas.

O requisito trata a prioridade das tarefas em uma fila de eventos significa que,
no momento de desbloquear uma tarefa localizada em uma fila de eventos, será
levando em consideração a sua prioridade. Assim, as tarefas com maiores
prioridades serão desbloqueadas primeiro do que as tarefas de menores
prioridades. Esse requisito será especificado através de alterações em nas
funções especificadas que trabalham com fila de eventos, como é o caso da
...
desbloqueadas e retiradas das filas de eventos.

Por fim, o requisito de tratamento de memória deve ser incorporado ao sistema.
Assim, no momento da criação de cada elemento a especificação poderá decidir se
há memória suficiente para a criação do mesmo ou não, retirando assim o
indeterminismo existente nas funcionalidades de criação de um elemento.


\section{Atividades e Etapas}\label{cap:cronograma}
Como demonstrado no capítulo~\ref{cap:primeiros_passos}, boa parte das atividades
necessárias para alcançar o objetivo desse trabalho já foram desenvolvida,
restando apenas as atividades discutidas como proposta nas seções anteriores.
Essas etapas, para um melhor entendimento, são resumidamente elencadas e
utilizadas na construção do cronograma do projeto(seção~\ref{sec:cronograma}) a
seguir.

\begin{enumerate}
\item Especificar, em um nível abstrato, as funcionalidades restantes dos
elementos tarefa e fila de mensagens.\label{etapa:funcionalidades}
\begin{itemize}
  \item Nessa etapa, serão desenvolvidas as funcionalidades da API do
  FreeRTOS, relacionadas aos elementos já especificados, que não foram tratadas
  na parte inicial do trabalho. 
\end{itemize}

\item Modelagem abstrata do elemento semáforo.\label{etapa:semaforo}
\begin{itemize}
  \item Nessa tarefa, o elemento semáforo (seção~\ref{sec:semaforo}) será
  especificado, de forma abstrata, junto com as funcionalidades da API do
  FreeRTOS que estão relacionadas a esse elemento.
\end{itemize} 

\item Modelagem abstrata do elemento mutex. \label{etapa:mutex}
\begin{itemize}
  \item Nessa parte do trabalho será especificado,
  de forma abstrata, o elemento mutex (seção~\ref{sec:mutex}), assim como as 
  funcionalidades da API do FreeRTOS que estão relacionadas com ele.
\end{itemize}

\item Modelagem abstrata do elemento co-rotina. \label{etapa:corotina}
\begin{itemize}
  \item Essa fase do trabalho encerra a especificação abstrata dos elementos do
  FreeRTOS. Nela, o elemento co-rotina(seção~\ref{sec:corotinas}) será modelado, em um
  nível abstrato, junto com suas funcionalidades. 
\end{itemize}

\item Refinamento do elemento tarefa.\label{etapa:refinamento_tarefa}
\begin{itemize}
  \item Nessa etapa, o elemento tarefa, especificado inicialmente, será
  refinado. Parte dessa etapa já foi iniciada nos primeiros passos desse
  trabalho(capítulo~\ref{cap:primeiros_passos}). Assim, resta apenas
  especificar as características de tempo bloqueio de uma tarefa, controle da
  execução de duas tarefas com a mesma prioridade, controle do estouro do
  tempo de execução e refinar as estruturas abstratas usada na especificação
  dessa entadide para um forma mais concretas.
\end{itemize}
 
\item Refinamento do elemento fila de mensagem.\label{etapa:refinamento_fila}
\begin{itemize}
  \item Como fila de mensagens tarefa foi especificado de forma abstrata
  inicialmente, essa etapa será responsável por realizar uma modelagem mais
  concreta desse elemento. Nessa etapa, serão tratadas as características de
  posição das mensagens e uma fila e prioridade das tarefas em uma fila de
  eventos.
  %, especificando mecânismo de modelagem utilizados, como o conjunto,
  %de uma forma mais próxima da implementação.  
\end{itemize}

\item Refinamento do elemento co-rotina.\label{etapa:refinamento_corotina}
\begin{itemize}
  \item Nessa fase ocorrerá o refinamento do elemento co-rotina, especificado
  as características de prioridade de uma co-rotina e tempo máximo de bloqueio
  de uma co-rotina.
%, tornando a sua modelagem mais concreta detalhada.
\end{itemize} 

\item Refinamento do elemento semáforo.\label{etapa:refinamento_semaforo}
\begin{itemize}
  \item Nessa parte do trabalho, o elemento semáforo,
  especificado de forma abstrata na etapa~\ref{etapa:semaforo}, será refinado.
  Através desse refinamento serão tratadas os requisitos ligados ao semáforo
  com contador.
\end{itemize}
  
\item Refinamento do elemento mutex.\label{etapa:refinamento_mutex}
\begin{itemize}
  \item Essa etapa encerra a especificação do FreeRTOS com o refinamento do
  elemento semáforo, especificado inicialmente na etapa~\ref{etapa:mutex}. Nela
  o tratamento de herança de prioridade será acrescentado à especificação.
\end{itemize}
...
\begin{itemize}
  \item Após desenvolvido o trabalho proposto, é hora de preparar a dissertação
  sobre o trabalho e finalmente o material para a defesa da dissertação, sendo
  este o trabalho dessa fase.
\end{itemize}\item
...
definidas, responsáveis por partes da especificação, que unidas formaram a
modelagem do sistema. Em seguida, utilizando o mecanismo de refinamento, os
módulos criados serão especificados de forma mais concreta, tornando assim a
especificação mais completa e próxima à realidade.
...
implementação atual usando ferramentas como FRAMA-C\cite{} ou VCC\cite{}. Por
fim, essa modelagem formal poderá servir como entrada para a criação de testes
de sistema a nível de codificação, criando-se assim mais uma forma de
verificação do correto funcionamento do FreeRTOS.
...
concretos a nível de linguagem algoritmicas e em seguida a adaptar tal modelagem
