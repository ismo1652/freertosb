...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Refinement(Task_block_time),t_beforeResumeAll)==(active = FALSE | tickCount,tickMissed:=TICK_INCREMENT(tickCount,tickMissed),0 || (@n_unblocked.(n_unblocked: FIN(TASK) & n_unblocked = {task | task: blocked & tickCount<=t_tblock(task) & t_tblock(task)<=TICK_INCREMENT(tickCount,tickMissed)} ==> unblocked:=n_unblocked) [] @n_unblocked.(n_unblocked: FIN(TASK) & n_unblocked = {task | task: blocked & (tickCount<=t_tblock(task) & t_tblock(task)<=MAXINT or t_tblock(task)<=TICK_INCREMENT(tickCount,tickMissed))} ==> unblocked:=n_unblocked)));
  ...
  List_Substitution(Refinement(Task_block_time),t_beforeResumeAll)==(tickCount:=TICK_INCREMENT(tickCount,tickMissed) || tickMissed:=0 || CHOICE ANY n_unblocked WHERE n_unblocked: FIN(TASK) & n_unblocked = {task | task: blocked & tickCount<=t_tblock(task) & t_tblock(task)<=TICK_INCREMENT(tickCount,tickMissed)} THEN unblocked:=n_unblocked END OR ANY n_unblocked WHERE n_unblocked: FIN(TASK) & n_unblocked = {task | task: blocked & (tickCount<=t_tblock(task) & t_tblock(task)<=MAXINT or t_tblock(task)<=TICK_INCREMENT(tickCount,tickMissed))} THEN unblocked:=n_unblocked END END);
