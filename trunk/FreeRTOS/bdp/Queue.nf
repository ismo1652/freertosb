...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(Queue),q_giveSemaphore)==(semaphore: semaphores | semaphore: semaphores_busy ==> semaphores_busy:=semaphores_busy-{semaphore} [] not(semaphore: semaphores_busy) ==> skip [] skip || @(n_receiving,n_first).(n_receiving: POW(TASK) & n_receiving = queue_receiving(semaphore)-{first_receiving(semaphore)} & n_first: TASK & n_first: n_receiving ==> queue_receiving,first_receiving:=queue_receiving<+{semaphore|->n_receiving},first_receiving<+{semaphore|->n_first}));
  ...
  List_Substitution(Machine(Queue),q_takeSemaphore)==(IF semaphore: semaphores_full THEN semaphores_full:=semaphores_full-{semaphore} ELSE CHOICE semaphores_busy:=semaphores_busy\/{semaphore} OR skip END END || ANY n_sending,n_first WHERE n_sending: POW(TASK) & n_sending = queue_sending(semaphore)-{first_sending(semaphore)} & n_first: TASK & n_first: n_sending THEN queue_sending(semaphore):=n_sending || first_sending(semaphore):=n_first END);
  List_Substitution(Machine(Queue),q_giveSemaphore)==(CHOICE IF semaphore: semaphores_busy THEN semaphores_busy:=semaphores_busy-{semaphore} END OR skip END || ANY n_receiving,n_first WHERE n_receiving: POW(TASK) & n_receiving = queue_receiving(semaphore)-{first_receiving(semaphore)} & n_first: TASK & n_first: n_receiving THEN queue_receiving(semaphore):=n_receiving || first_receiving(semaphore):=n_first END);
