...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Machine(Queue))==(mutex_holder,queue_sending,queue_receiving,queue_items,mutexes_busy,mutexes,semaphores_busy,semaphores,queues_msg,queues);
  List_Variables(Machine(Queue))==(mutex_holder,queue_sending,queue_receiving,queue_items,mutexes_busy,mutexes,semaphores_busy,semaphores,queues_msg,queues);
  External_List_Variables(Machine(Queue))==(mutex_holder,queue_sending,queue_receiving,queue_items,mutexes_busy,mutexes,semaphores_busy,semaphores,queues_msg,queues)
END
&
...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(Queue))==(queues: POW(QUEUE) & queue_receiving: QUEUE +-> POW(TASK) & queue_sending: QUEUE +-> POW(TASK) & queues = dom(queue_receiving) & queues = dom(queue_sending) & queues_msg <: queues & queue_items: QUEUE +-> POW(ITEM) & queues_msg = dom(queue_items) & !(q1,q2,tk).(q1: queues & q2: queues & tk: TASK & tk: queue_receiving(q1) => tk/:queue_sending(q2)) & !(q1,q2,tk).(q1: queues & q2: queues & tk: TASK & tk: queue_sending(q2) => tk/:queue_receiving(q1)) & !(q1,q2,tk).(q1: queues & q2: queues & q1/=q2 & tk: TASK & tk: queue_receiving(q1) => tk/:queue_receiving(q2)) & !(q1,q2,tk).(q1: queues & q2: queues & q1/=q2 & tk: TASK & tk: queue_sending(q1) => tk/:queue_sending(q2)) & semaphores: POW(QUEUE) & semaphores <: queues & semaphores_busy <: semaphores & mutexes <: queues & mutex_holder: QUEUE +-> TASK & mutexes = dom(mutex_holder) & mutexes_busy <: mutexes & !mt.(mt: mutexes_busy => mutex_holder(mt)/=TASK_NULL) & !mt.(mt: mutexes & mt/:mutexes_busy => mutex_holder(mt) = TASK_NULL))
END
&
THEORY ListAssertionsX IS
  ...
  List_Assertions(Machine(Queue))==(!que.(que: QUEUE & que/:queues => que/:mutexes) & !que.(que: QUEUE & que/:queues => que/:semaphores))
END
&
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(Queue))==(queues,queues_msg,queue_items,queue_receiving,queue_sending,semaphores,semaphores_busy,mutexes,mutexes_busy,mutex_holder:={},{},{},{},{},{},{},{},{},{});
  Context_List_Initialisation(Machine(Queue))==(skip);
  List_Initialisation(Machine(Queue))==(queues:={} || queues_msg:={} || queue_items:={} || queue_receiving:={} || queue_sending:={} || semaphores:={} || semaphores_busy:={} || mutexes:={} || mutexes_busy:={} || mutex_holder:={})
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Machine(Queue))==(q_queueCreate,q_queueDelete,q_sendItem,q_receivedItem,q_insertTaskWaitingToSend,q_insertTaskWaitingToReceive,q_removeFromEventListQueue,q_createSemaphore,q_takeSemaphore,q_giveSemaphore,q_createMutex,q_takeMutex,q_giveMutex,q_endScheduler,q_resumeAll);
  List_Operations(Machine(Queue))==(q_queueCreate,q_queueDelete,q_sendItem,q_receivedItem,q_insertTaskWaitingToSend,q_insertTaskWaitingToReceive,q_removeFromEventListQueue,q_createSemaphore,q_takeSemaphore,q_giveSemaphore,q_createMutex,q_takeMutex,q_giveMutex,q_endScheduler,q_resumeAll)
END
&
THEORY ListInputX IS
  ...
  List_Input(Machine(Queue),q_giveMutex)==(mutex,pxTask,run_task);
...
THEORY ListHeaderX IS
  ...
  List_Header(Machine(Queue),q_giveMutex)==(q_giveMutex(mutex,pxTask,run_task));
...
THEORY ListPreconditionX IS
  List_Precondition(Machine(Queue),q_queueCreate)==(uxQueueLength: NAT & uxItemSize: NAT);
  ...
  List_Precondition(Machine(Queue),q_sendItem)==(pxQueue: queues_msg & pxItem: ITEM & task: TASK & copy_position: COPY_POSITION & task: queue_receiving(pxQueue));
  List_Precondition(Machine(Queue),q_receivedItem)==(pxQueue: queues_msg & justPeeking: BOOL & task: TASK & task: queue_sending(pxQueue) & queue_items(pxQueue)/={});
  ...
  List_Precondition(Machine(Queue),q_createSemaphore)==(maxCount: INT & initialCount: INT & initialCount<=maxCount);
  ...
  List_Precondition(Machine(Queue),q_giveMutex)==(mutex: mutexes & mutex: mutexes_busy & pxTask: TASK & run_task = mutex_holder(mutex));
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(Queue),q_endScheduler)==(btrue | queues,queues_msg,queue_items,queue_receiving,queue_sending,semaphores,semaphores_busy,mutexes,mutexes_busy,mutex_holder:={},{},{},{},{},{},{},{},{},{});
  Expanded_List_Substitution(Machine(Queue),q_giveMutex)==(mutex: mutexes & mutex: mutexes_busy & pxTask: TASK & run_task = mutex_holder(mutex) | mutexes_busy,mutex_holder,queue_receiving:=mutexes_busy-{mutex},mutex_holder<+{mutex|->TASK_NULL},queue_receiving<+{mutex|->queue_receiving(mutex)-{pxTask}});
  ...
  Expanded_List_Substitution(Machine(Queue),q_giveSemaphore)==(semaphore: semaphores & pxTask: TASK & pxTask: queue_receiving(semaphore) | semaphore: semaphores_busy ==> semaphores_busy,queue_receiving:=semaphores_busy-{semaphore},queue_receiving<+{semaphore|->queue_receiving(semaphore)-{pxTask}} [] not(semaphore: semaphores_busy) ==> skip);
  ...
  Expanded_List_Substitution(Machine(Queue),q_createSemaphore)==(maxCount: INT & initialCount: INT & initialCount<=maxCount | @semaphore.(semaphore: QUEUE & semaphore/:queues ==> (queues,queue_receiving,queue_sending,semaphores:=queues\/{semaphore},queue_receiving\/{semaphore|->{}},queue_sending\/{semaphore|->{}},semaphores\/{semaphore} || (semaphores_busy:=semaphores_busy\/{semaphore} [] skip) || rSemaphore:=semaphore)));
  ...
  Expanded_List_Substitution(Machine(Queue),q_receivedItem)==(pxQueue: queues_msg & justPeeking: BOOL & task: TASK & task: queue_sending(pxQueue) & queue_items(pxQueue)/={} | @item.(item: ITEM & item: queue_items(pxQueue) ==> (justPeeking = FALSE ==> queue_items,queue_sending:=queue_items<+{pxQueue|->queue_items(pxQueue)-{item}},queue_sending<+{pxQueue|->queue_sending(pxQueue)-{task}} [] not(justPeeking = FALSE) ==> skip || xItem:=item)));
  Expanded_List_Substitution(Machine(Queue),q_sendItem)==(pxQueue: queues_msg & pxItem: ITEM & task: TASK & copy_position: COPY_POSITION & task: queue_receiving(pxQueue) | queue_items,queue_receiving:=queue_items<+{pxQueue|->(queue_items(pxQueue)\/{pxItem})},queue_receiving<+{pxQueue|->queue_receiving(pxQueue)-{task}});
  Expanded_List_Substitution(Machine(Queue),q_queueDelete)==(pxQueue: queues & pxQueue/:semaphores & pxQueue/:mutexes & queue_receiving(pxQueue) = {} & queue_sending(pxQueue) = {} | queues,queues_msg,queue_items,queue_receiving,queue_sending:=queues-{pxQueue},queues_msg-{pxQueue},{pxQueue}<<|queue_items,{pxQueue}<<|queue_receiving,{pxQueue}<<|queue_sending);
  Expanded_List_Substitution(Machine(Queue),q_queueCreate)==(uxQueueLength: NAT & uxItemSize: NAT | @pxQueue.(pxQueue: QUEUE & pxQueue/:queues ==> queues,queues_msg,queue_items,queue_receiving,queue_sending,xQueueHandle:=queues\/{pxQueue},queues_msg\/{pxQueue},queue_items\/{pxQueue|->{}},queue_receiving\/{pxQueue|->{}},queue_sending\/{pxQueue|->{}},pxQueue));
  List_Substitution(Machine(Queue),q_queueCreate)==(ANY pxQueue WHERE pxQueue: QUEUE & pxQueue/:queues THEN queues:=queues\/{pxQueue} || queues_msg:=queues_msg\/{pxQueue} || queue_items:=queue_items\/{pxQueue|->{}} || queue_receiving:=queue_receiving\/{pxQueue|->{}} || queue_sending:=queue_sending\/{pxQueue|->{}} || xQueueHandle:=pxQueue END);
  List_Substitution(Machine(Queue),q_queueDelete)==(queues:=queues-{pxQueue} || queues_msg:=queues_msg-{pxQueue} || queue_items:={pxQueue}<<|queue_items || queue_receiving:={pxQueue}<<|queue_receiving || queue_sending:={pxQueue}<<|queue_sending);
  ...
  List_Substitution(Machine(Queue),q_receivedItem)==(ANY item WHERE item: ITEM & item: queue_items(pxQueue) THEN IF justPeeking = FALSE THEN queue_items(pxQueue):=queue_items(pxQueue)-{item} || queue_sending(pxQueue):=queue_sending(pxQueue)-{task} END || xItem:=item END);
  ...
  List_Substitution(Machine(Queue),q_giveSemaphore)==(IF semaphore: semaphores_busy THEN semaphores_busy:=semaphores_busy-{semaphore} || queue_receiving(semaphore):=queue_receiving(semaphore)-{pxTask} END);
  ...
  List_Substitution(Machine(Queue),q_endScheduler)==(queues:={} || queues_msg:={} || queue_items:={} || queue_receiving:={} || queue_sending:={} || semaphores:={} || semaphores_busy:={} || mutexes:={} || mutexes_busy:={} || mutex_holder:={});
...
THEORY ListSeenInfoX IS
  ...
  Seen_Context_List_Assertions(Machine(Queue))==(BIT <: NATURAL);
...
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(Queue)) == (remove_task | ? | mutex_holder,queue_sending,queue_receiving,queue_items,mutexes_busy,mutexes,semaphores_busy,semaphores,queues_msg,queues | ? | q_queueCreate,q_queueDelete,q_sendItem,q_receivedItem,q_insertTaskWaitingToSend,q_insertTaskWaitingToReceive,q_removeFromEventListQueue,q_createSemaphore,q_takeSemaphore,q_giveSemaphore,q_createMutex,q_takeMutex,q_giveMutex,q_endScheduler,q_resumeAll | ? | seen(Machine(Types)),seen(Machine(Scheduler)) | ? | Queue);
...
THEORY VariablesEnvX IS
  Variables(Machine(Queue)) == (Type(mutex_holder) == Mvl(SetOf(atype(QUEUE,?,?)*atype(TASK,?,?)));Type(queue_sending) == Mvl(SetOf(atype(QUEUE,?,?)*SetOf(atype(TASK,?,?))));Type(queue_receiving) == Mvl(SetOf(atype(QUEUE,?,?)*SetOf(atype(TASK,?,?))));Type(queue_items) == Mvl(SetOf(atype(QUEUE,?,?)*SetOf(atype(ITEM,?,?))));Type(mutexes_busy) == Mvl(SetOf(atype(QUEUE,?,?)));Type(mutexes) == Mvl(SetOf(atype(QUEUE,?,?)));Type(semaphores_busy) == Mvl(SetOf(atype(QUEUE,?,?)));Type(semaphores) == Mvl(SetOf(atype(QUEUE,?,?)));Type(queues_msg) == Mvl(SetOf(atype(QUEUE,?,?)));Type(queues) == Mvl(SetOf(atype(QUEUE,?,?))))
END
&
THEORY OperationsEnvX IS
  Operations(Machine(Queue)) == (Type(q_resumeAll) == Cst(No_type,SetOf(atype(TASK,?,?)));Type(q_endScheduler) == Cst(No_type,No_type);Type(q_giveMutex) == Cst(No_type,atype(QUEUE,?,?)*atype(TASK,?,?)*atype(TASK,?,?));Type(q_takeMutex) == Cst(No_type,atype(QUEUE,?,?)*atype(TASK,?,?));Type(q_createMutex) == Cst(atype(QUEUE,?,?),No_type);Type(q_giveSemaphore) == Cst(No_type,atype(QUEUE,?,?)*atype(TASK,?,?));Type(q_takeSemaphore) == Cst(No_type,atype(QUEUE,?,?)*atype(TASK,?,?));Type(q_createSemaphore) == Cst(atype(QUEUE,?,?),btype(INTEGER,?,?)*btype(INTEGER,?,?));Type(q_removeFromEventListQueue) == Cst(No_type,atype(TASK,?,?));Type(q_insertTaskWaitingToReceive) == Cst(No_type,atype(QUEUE,?,?)*atype(TASK,?,?));Type(q_insertTaskWaitingToSend) == Cst(No_type,atype(QUEUE,?,?)*atype(TASK,?,?));Type(q_receivedItem) == Cst(atype(ITEM,?,?),atype(QUEUE,?,?)*btype(BOOL,?,?)*atype(TASK,?,?));Type(q_sendItem) == Cst(No_type,atype(QUEUE,?,?)*atype(ITEM,?,?)*atype(TASK,?,?)*etype(COPY_POSITION,?,?));Type(q_queueDelete) == Cst(No_type,atype(QUEUE,?,?));Type(q_queueCreate) == Cst(atype(QUEUE,?,?),btype(INTEGER,?,?)*btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
