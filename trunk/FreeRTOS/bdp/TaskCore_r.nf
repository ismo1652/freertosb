...
THEORY ListInvariantX IS
  ...
  List_Invariant(Refinement(TaskCore_r))==(active: BOOL & t_priority: TASK +-> PRIORITY & dom(t_priority) = tasks & (active = TRUE => t_priority(idle) = IDLE_PRIORITY & !tt.(tt: ready => t_priority(tt)<=t_priority(running)) & !tt.(tt: tasks => IDLE_PRIORITY<=t_priority(tt))))
END
&
