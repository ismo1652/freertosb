...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(Task_Basic),vTaskStartScheduler)==(scheduler = taskSCHEDULER_NOT_STARTED | @(idle_name,idle_task).(idle_name: NAME & idle_task: TASK & idle_task/:tasks ==> (tasks,task_name,task_priority,task_value:=tasks\/{idle_task},task_name\/{idle_task|->idle_name},task_priority\/{idle_task|->tskIDLE_PRIORITY},task_value\/{idle_task|->0} || (tasks = {TASK_NULL} ==> task_state:=task_state<+{idle_task|->running} [] not(tasks = {TASK_NULL}) ==> task_state:=task_state<+{idle_task|->ready}))) || tickCount:=0 || tickMissed:=0 [] skip);
  Expanded_List_Substitution(Machine(Task_Basic),xTaskGetTickCount)==(INCLUDE_xTaskGetCurrentTaskHandle = 1 | zz:=tickCount);
  Expanded_List_Substitution(Machine(Task_Basic),vTaskDelayUntil)==(pxPreviousWakeTime: TICK & xTimeIncrement: TICK & INCLUDE_vTaskDelayUntil = 1 | @(mprt,ctk).(mprt: PRIORITY & mprt = max(READY_PRIORITIES(task_state,task_priority)) & ctk: dom({current_task}<<|task_priority) & task_priority(ctk) = mprt & task_state(ctk) = ready ==> task_state,task_value,current_task,max_priority:=task_state<+{current_task|->blocked}<+{ctk|->running},task_value<+{current_task|->TICK_INCREMENT(pxPreviousWakeTime,xTimeIncrement)},ctk,mprt) [] skip);
  Expanded_List_Substitution(Machine(Task_Basic),vTaskDelay)==(xTicksToDelay: TICK & not(xTicksToDelay = 0) & INCLUDE_vTaskDelay = 1 | @(mprt,ctk).(mprt: PRIORITY & mprt = max(READY_PRIORITIES(task_state,task_priority)) & ctk: dom({current_task}<<|task_priority) & task_priority(ctk) = mprt & task_state(ctk) = ready ==> task_state,task_value,current_task,max_priority:=task_state<+{current_task|->blocked}<+{ctk|->running},task_value<+{current_task|->TICK_INCREMENT(tickCount,xTicksToDelay)},ctk,mprt));
  ...
  Expanded_List_Substitution(Machine(Task_Basic),vTaskSuspend)==(INCLUDE_vTaskSuspend = 1 & pxTaskToSuspend: tasks | not(task_state(pxTaskToSuspend) = suspended) ==> (pxTaskToSuspend = current_task ==> @(mprt,ctk).(mprt: PRIORITY & mprt = max(READY_PRIORITIES(task_state,task_priority)) & ctk: dom({pxTaskToSuspend}<<|task_priority) & task_priority(ctk) = mprt & task_state(ctk) = ready ==> task_state,current_task,max_priority:=task_state<+{pxTaskToSuspend|->suspended}<+{ctk|->running},ctk,mprt) [] not(pxTaskToSuspend = current_task) ==> task_state:=task_state<+{pxTaskToSuspend|->suspended} || rr:=pdTRUE) [] not(not(task_state(pxTaskToSuspend) = suspended)) ==> rr:=pdFALSE);
  Expanded_List_Substitution(Machine(Task_Basic),deleteTask)==(INCLUDE_vTaskDelete = 1 & pxTaskToDelete: tasks & task_state(pxTaskToDelete)/=deleted | tasks,task_name:=tasks-{pxTaskToDelete},{pxTaskToDelete}<<|task_name || (pxTaskToDelete = current_task ==> @(mpriority,ctk).(mpriority: PRIORITY & mpriority = max(READY_PRIORITIES(task_state,task_priority)) & ctk: dom({pxTaskToDelete}<<|task_priority) & task_priority(ctk) = mpriority & task_state(ctk) = ready ==> task_state,max_priority,current_task:={current_task}<<|task_state<+{ctk|->running},mpriority,ctk) [] not(pxTaskToDelete = current_task) ==> task_state:={pxTaskToDelete}<<|task_state) || task_priority:={pxTaskToDelete}<<|task_priority || task_value:={pxTaskToDelete}<<|task_value);
  Expanded_List_Substitution(Machine(Task_Basic),xTaskCreate)==(pvTaskCode: TASK_CODE & pcName: NAME & usStackDepth: NATURAL & pvParameters <: PARAMETER & uxPriority: PRIORITY | @task.(task: TASK & task/:tasks ==> (tasks,task_name,task_value:=tasks\/{task},task_name\/{task|->pcName},task_value\/{task|->0} || (uxPriority>max_priority ==> task_state,max_priority,current_task:=task_state<+{current_task|->ready,task|->running},uxPriority,task [] not(uxPriority>max_priority) ==> task_state:=task_state<+{task|->ready}) || task_priority:=task_priority\/{task|->uxPriority} || zz,handle:=pdPASS,task)) [] zz,handle:=errCOULD_NOT_ALLOCATE_REQUIRED_MEMORY,TASK_NULL);
