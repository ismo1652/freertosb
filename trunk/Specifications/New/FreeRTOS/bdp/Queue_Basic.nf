...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Machine(Queue_Basic),removeFromEventListQueue)==(task: TASK | @(queuesTask,q_tkRecived,q_tkSend).(queuesTask: POW(QUEUE) & queuesTask = QUEUES_WITH_TASK(task,queues,queue_tkRecived,queue_tkSend) & q_tkRecived: QUEUE +-> POW(TASK) & q_tkSend: QUEUE +-> POW(TASK) & q_tkRecived,q_tkSend = REMOVE_EVENT(task,queues,queue_tkRecived,queue_tkSend) ==> queue_tkRecived,queue_tkSend:=queue_tkRecived<+q_tkRecived,queue_tkSend<+q_tkSend));
  Expanded_List_Substitution(Machine(Queue_Basic),recivedItem)==(pxQueue: queues & QUEUE_ISEMPTY(pxQueue,queue_itens) = FALSE & justPeeking: BIT & task: TASK & task: queue_tkSend(pxQueue) | @item.(item: ITEM & item: queue_itens(pxQueue) ==> (justPeeking = pdFALSE ==> queue_itens,queue_tkSend:=queue_itens<+{pxQueue|->queue_itens(pxQueue)-{item}},queue_tkSend<+{pxQueue|->queue_tkSend(pxQueue)-{task}} [] not(justPeeking = pdFALSE) ==> skip || xItem:=item)));
...
THEORY ListSeenInfoX IS
  ...
  Seen_Expanded_List_Substitution(Machine(Queue_Basic),incrementTick)==(scheduler = taskSCHEDULER_RUNNING ==> tickCount:=TICK_INCREMENT(tickCount,1) [] not(scheduler = taskSCHEDULER_RUNNING) ==> tickMissed:=TICK_INCREMENT(tickCount,1) || @(blockedTasks,mprt,task).(blockedTasks = {bTask | bTask: tasks & task_state(bTask) = blocked & task_value(bTask) = TICK_INCREMENT(tickCount,1)} & mprt: PRIORITY & mprt = max(ran(dom(UNBLOCK_TASKS(blockedTasks))<|task_priority)) & task: dom(UNBLOCK_TASKS(blockedTasks)) & task: dom(task_priority) & task_priority(task) = mprt ==> (mprt>max_priority ==> task_state,current_task,max_priority:=task_state<+UNBLOCK_TASKS(blockedTasks)<+{current_task|->ready,task|->running},task,mprt [] not(mprt>max_priority) ==> task_state:=task_state<+UNBLOCK_TASKS(blockedTasks))));
  Seen_List_Precondition(Machine(Queue_Basic),xTaskResumeAll)==(btrue);
  Seen_Expanded_List_Substitution(Machine(Queue_Basic),xTaskResumeAll)==(scheduler = taskSCHEDULER_SUSPENDED ==> (scheduler,tickCount:=taskSCHEDULER_RUNNING,TICK_INCREMENT(tickCount,tickMissed) || @(blockedTasks,mprt,task).(blockedTasks = {bTask | bTask: tasks & task_state(bTask) = blocked & tickCount<=task_value(bTask) & task_value(bTask)<=TICK_INCREMENT(tickCount,tickMissed)} & mprt: PRIORITY & mprt = max(ran(dom(UNBLOCK_TASKS(blockedTasks))<|task_priority)) & task: dom(UNBLOCK_TASKS(blockedTasks)) & task: dom(task_priority) & task_priority(task) = mprt ==> (mprt>max_priority ==> task_state,current_task,max_priority:=task_state<+UNBLOCK_TASKS(blockedTasks)<+{current_task|->ready,task|->running},task,mprt [] not(mprt>max_priority) ==> task_state:=task_state<+UNBLOCK_TASKS(blockedTasks)))) [] not(scheduler = taskSCHEDULER_SUSPENDED) ==> skip);
  ...
  Seen_Expanded_List_Substitution(Machine(Queue_Basic),vTaskStartScheduler)==(@(idle_name,idle_task).(idle_name: NAME & idle_task: TASK & idle_task/:tasks ==> (tasks,task_name,task_priority,task_value:=tasks\/{idle_task},task_name\/{idle_task|->idle_name},task_priority\/{idle_task|->tskIDLE_PRIORITY},task_value\/{idle_task|->0} || (tasks = {TASK_NULL} ==> task_state:=task_state<+{idle_task|->running} [] not(tasks = {TASK_NULL}) ==> task_state:=task_state<+{idle_task|->ready}))) || tickCount:=0 || tickMissed:=0 [] skip);
  ...
  Seen_Expanded_List_Substitution(Machine(Queue_Basic),vTaskDelayUntil)==(@(mprt,ctk).(mprt: PRIORITY & mprt = max(READY_PRIORITIES(task_state,task_priority)) & ctk: dom({current_task}<<|task_priority) & task_priority(ctk) = mprt & task_state(ctk) = ready ==> task_state,task_value,current_task,max_priority:=task_state<+{current_task|->blocked}<+{ctk|->running},task_value<+{current_task|->TICK_INCREMENT(pxPreviousWakeTime,xTimeIncrement)},ctk,mprt) [] skip);
  Seen_List_Precondition(Machine(Queue_Basic),vTaskDelay)==(xTicksToDelay: TICK & not(xTicksToDelay = 0) & INCLUDE_vTaskDelay = 1);
  Seen_Expanded_List_Substitution(Machine(Queue_Basic),vTaskDelay)==(@(mprt,ctk).(mprt: PRIORITY & mprt = max(READY_PRIORITIES(task_state,task_priority)) & ctk: dom({current_task}<<|task_priority) & task_priority(ctk) = mprt & task_state(ctk) = ready ==> task_state,task_value,current_task,max_priority:=task_state<+{current_task|->blocked}<+{ctk|->running},task_value<+{current_task|->TICK_INCREMENT(tickCount,xTicksToDelay)},ctk,mprt));
  ...
  Seen_Expanded_List_Substitution(Machine(Queue_Basic),deleteTask)==(tasks,task_name:=tasks-{pxTaskToDelete},{pxTaskToDelete}<<|task_name || (pxTaskToDelete = current_task ==> @(mpriority,ctk).(mpriority: PRIORITY & mpriority = max(READY_PRIORITIES(task_state,task_priority)) & ctk: dom({pxTaskToDelete}<<|task_priority) & task_priority(ctk) = mpriority & task_state(ctk) = ready ==> task_state,max_priority,current_task:={current_task}<<|task_state<+{ctk|->running},mpriority,ctk) [] not(pxTaskToDelete = current_task) ==> task_state:={pxTaskToDelete}<<|task_state) || task_priority:={pxTaskToDelete}<<|task_priority || task_value:={pxTaskToDelete}<<|task_value);
  ...
  Seen_Expanded_List_Substitution(Machine(Queue_Basic),xTaskCreate)==(@task.(task: TASK & task/:tasks ==> (tasks,task_name,task_value:=tasks\/{task},task_name\/{task|->pcName},task_value\/{task|->0} || (uxPriority>max_priority ==> task_state,max_priority,current_task:=task_state<+{current_task|->ready,task|->running},uxPriority,task [] not(uxPriority>max_priority) ==> task_state:=task_state<+{task|->ready}) || task_priority:=task_priority\/{task|->uxPriority} || zz,handle:=pdPASS,task)) [] zz,handle:=errCOULD_NOT_ALLOCATE_REQUIRED_MEMORY,TASK_NULL);
