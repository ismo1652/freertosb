...
na sua portabilidade, sendo o mesmo oficialmente disponível em 17
arquiteturas diferentes, entre elas a PIC, ARM e Zilog Z80, as quais
são muito difundidas em produtos comerciais através de sistemas
computacionais embutidos.

O FreeRTOS provê acesso a recursos de \textit{hardware}, permitindo o
desenvolvimento de sistemas concorrentes de tempo-real. Assim, o
FreeRTOS fornece uma camada de abstração, que tem como papel esconder
dos desenvolvedores de aplicações os detalhes do hardware que será
...
O FreeRTOS é disponibilizada sob a forma de uma biblioteca de tipos e
funções que deve ser linkeditada com o código da aplicação a ser
desenvolvido. Tipicamente esse código é divido em duas partes:
primeiro o código das tarefas que estarão executando durante a
operação do sistema, e segundo um código responsável pela
inicialização do sistema, o qual deverá criar e cadastrar as tarefas e
então dar início ao escalonador.
...
\item uma pilha de contexto, que armazena o ambiente de execução
  (estados dos registradores) quando a tarefa é interrompida;
  %% DD->SG: ATE ONDE EU SEI, PILHA DE CONTEXTO E OUTRA COISA (PILHA
  %% DAS CHAMADAS DE FUNCAO ATIVAS). VERIFIQUE O QUE ESTA CERTO NESSE
  %% CONTEXTO.

\end{itemize}

O FreeRTOS, além de permitir a execução de aplicações multitarefa, dá
suporte a três tipos de configurações diferentes de gerenciamento de
tarefas: preemptivo, cooperativo ou híbrido. 
...
execução não é interrompida pelo escalonador, pois ela é quem decide o
momento de sua pausa seguida da sua substituição, que será feita de
...
A política de escalonamento do FreeRTOS, assim como a maioria dos
sistemas operacionais de tempo real, é baseada na prioridade. Isso
indica que tarefas com maiores prioridades têm sempre a preferência
sobre as tarefas de menores prioridades do mesmo estado.  Uma
conseqüência dessa política é que a tarefa que está em execução deve
possuir prioridade maior ou igual a das tarefas de estado pronto, pois
a tarefa em execução nada mais é do que a tarefa que tem preferência
de entrar em execução entre as tarefas de estado pronto.

Uma outra característica importante do escalonador é que distribui o
tempo de processamento entre tarefas de mesma propridade. Com isso, se
houver duas, ou mais, tarefas prontas e com a prioridade máxima, elas
irão dividir o tempo de processamento.

\subsection{Comunicação e sincronização entre tarefas}

A comunicação entre tarefas no FreeRTOS é feita através de filas de
mensagens. Tarefas podem enviar a uma fila ou ler mensagens de uma
fila. No FreeRTOS, as filas de mensagens possuem uma capacidade
limitada e fixa, determinada na criação da fila. As mensagens também
possuem um tamanho fixo.

Para as tarefas, o acesso às filas de mensagens é bloqueador: auando
uma tarefa envia uma mensagem para uma fila, e está não estiver
lotada, a mensagem é inserida na fila, caso contrário, a tarefa
permanece bloqueada até que seja liberado um lugar na fila. O mesmo
ocorre quando uma tarefa quer ler uma mensagem da fila. Se a fila
estiver vazia, a tarefa permanece bloquada até uma outra tarefa enviar
uma mensagem nesta fila. Se a fila não estiver vazia, a leitura é
realizada (retornando a mensagem mais antiga na fila).

No FreeRTOS, os semáforos formam o principal serviço de sincronização
para o acesso a recursos compartilhados. São implementados como uma
fila de mensagens com um único item. Por convenção, o semáforo está em
uso, e livro quando a fila estiver cheia. No caso de filas implementando
semáforos, o conteúdo das mensagens não é relevante, apenas o estado da
fila.

O FreeRTOS oferece uma segunda primitiva de sincronização, chamada
mutex, a qual implementa uma técnica chamada de herança de
prioridade. Um mutex nada mais é que um semáforo com uma prioridade
associada, a qual é igual à da tarefa que usa aquele mutex no momento
(ou zero, se não houver). No acesso a um mutex ocupado, se a tarefa
solicitante tiver prioridade maior que a prioridade associada a este
mutex, a tarefa proprietária do mutex tem a sua prioridade elevada ao
nível (herda) da tarefa solicitante. Essa tarefa, momentaneamente
promovida, tem assim acesso ao processodor facilitado, o que pode
garantir uma liberação mais rápida do mutex. Esse mecanismo permite
evitar o fenômeno de intertravamento de tarefas chamado de prioridade
de inversão.

%% DD->SG: INCLUIR REFERENCIA BIBLIOGRAFICA SOBRE PRIORIDADE DE
%% INVERSAO
...
descritas em sua especificação. O desenvolvimento é realizado da
...
O desenvolvimento em B consiste em escrever módulos em diferentes
níveis de abstração e a efetuar análise estática desses módulos. Os
módulos são desenvolvidos usando uma linguagem denominada
\emph{notação de máquina abstrata}.  Cada módulo é composto de
diferentes seções, cada qual é responsável por definir uma aspecto da
especificação do sistema como: parâmetros, tipos, constantes,
variáveis de estado, estados iniciais e transições do sistema. Como
exemplo, a figura~\ref{fig:tarefas_maquina} contem um módulo, chamado
\textit{Kernel}, o qual especifica um sistema que permite incluir e
excluir tarefas até o limite de 10 tarefas. As seções desse módulo
são:
\begin{description}
...
necessita a verificação de condições lógicas. No método B, essas
condições são expressas como fórmulas em lógica da primeira ordem e o
usuário deve provar que essas fórmulas são válidas. Para o exemplo
...
refinamentos permites ou complementar uma especificação, ou
...
É seguindo esse raciocínio que o método B especifica constrói sistemas
...
A modelagem formal do FreeRTOS foi precedida de uma etapa de estudo 
visando ganhar familiaridade com o sistema. Nessa etapa, observou-se
que a estruturação dos tipos e funções da biblioteca permitiria um
desenvolvimento incremento dessa modelagem. 
...
são que casos particulares de filas de mensagens.
...
salientar que a composição dessas duas entidades já resulte em um
núcleo completo de sistema operacional. De fato, a entidade ``tarefa''
...
não só a comunicação entre tarefas, mas também a sua sincronização
no acesso a recursos compartilhados.

\subsection{Tarefa}

%% DD->SG - Acho que não cabe falar de implementação em uma seção
%% dedicada ao projeto de modelagem. Se for necessário entrear nesses
%% detalhes, eles deveriam estar na seção sobre FreeRTOS. No entanto,
%% eu não acho que devamos entrar nesses detalhes.

% Na sessão \ref{subsec:gerenciamento_tarefa}, foi dito que uma tarefa é
% composta por uma prioridade, por um estado, e uma pilha de
% contexto. Assim, no FreeRTOS, a implementação de uma tarefa é feita
% como demonstra o código a seguir, no qual primeiramente é criada a
% variável \textit{pxCurrentTCB}, que armazena uma referência para a
% tarefa em execução, e as listas de tarefas \textit{ReadyTasksLists},
% \textit{pxDelayedTaskList} e \textit{xSuspendedTaskList}, que
% armazenam respectivamente referências para tarefas no estado pronto,
% bloqueado e suspenso. Após isso, é criado um bloco de controle
% reponsável pela representação de uma tarefa para o sistema. Assim esse
% bloco é formado principalmente pela variável
% \textit{xGenericListItem}, que é colocada nas listas e variável de
% estados, indicando qual é o estado da tarefa, pela variável
% \textit{uxPriority} que armazena a prioridade da tarefa e pela
% variável \textit{pxTopOfStack} que aponta para o começo da pilha de
...
Dois tipos de funções manipulam tarefas: gerenciamento de tarefa e
controle do escalonador. A seguir tem-se a listagem e descrição de
cada uma das funcionalidades desses grupos escolhidas para fazerem
parte da modelagem inicial.

As funções de gerenciamento de tarefas são responsáveis por criar,
excluir e manusear o conceito de tarefa. Tem no total 10 funções,
sendo que oito foram elencadas para fazer parte do modelo inicial.
São elas:

\begin{description}
  
\item{xTaskCreate:} cria uma nova tarefa;
\item{xTaskDelete:} remove uma tarefa do FreeRTOS;
\item{uxTaskPriorityGet:} informa a prioridade de uma tarefa;
\item{vTaskSuspend:} coloca uma tarefa no estado ``suspenso'';
\item{vTaskResume:} retorna uma tarefa suspensa colocando-a no estado
  pronto;
\item{vTaskPrioritySet:} altera a prioridade de uma tarefa;
\item{vTaskDelay:} bloqueia uma tarefa por uma determinada
  quantidade de tempo, levando em consideração o instante em que o método foi
  chamada;
\item{vTaskDelayUntil:} bloqueia uma tarefa por uma determinada
  quantidade de tempo, levando em consideração o instante em que essa
  foi desbloqueada.
\end{description}


As funcionalidades de controle do escalonador são responsáveis por
iniciar, finalizar, suspender, retornar e fornecer informações úteis
sobre as atividades do escalonador em particular, e do núcleo do
sistema em geral. Foram selecionadas as seguintes funções:

\begin{itemize}
\item{xTaskGetCurrentTaskHandle:} retorna uma referência para a
  tarefa em execução;
\item{xTaskGetSchedulerState:} informa o atual estado do escalonador
  (executando, suspenso ou não inicializado);
\item{uxTaskGetNumberOfTasks:} informa a atual quantidade de tarefas
  tarefas do FreeRTOS;
\item{xTaskGetTickCount:} informa o tempo decorrido apartir da
  inicialização do escalonador;
\item{vTaskStartScheduler:} esse método primeiramente cria a tarefa
  ociosa e, em seguida, inicia as atividades do escalonador;
\item{vTaskEndScheduler:} finaliza as atividades do escalonador
...
\item{vTaskSuspendAll:} suspende temporariamente todas atividades do
  escalonador colocando-o no estado suspenso;
\item{xTaskResumeAll:} retorna todas as atividades do escalonador
  quando este encontra-se no estado suspenso.
\end{itemize}

\subsection{Fila de mensagens}

%% DD->SG: vide comentário anterior.

% O conceito de fila de mensagens é implementado como demonstra o código
% abaixo. Ele é formado principalmente por três variáveis. A primeira
% variável \textit{pcHead} aponta para o início de uma área de memória
% alocada na criação da fila para armazenadas as mensagem enviadas à
% fila. Na segunda, \textit{xTasksWaitingToReceive}, são armazenadas
% referências paras tarefas que aguardam pela chegada de uma mensagem na
% fila, ou seja, as tarefas que foram bloqueadas ao tentar ler a fila
% quando ela estava vazia. E por último, na terceira variável
% \textit{xTasksWaitingToSend}, são armazenadas referências para as
% tarefas que aguardam para enviar uma mensagem para a fila, ou seja, as
% tarefas que foram bloqueadas ao tentaram enviar uma mensagem para a
% fila quando esta estava cheia. Além disso, na implementação desse
% conceito existem também duas variáveis adicionais \textit{uxItemSize}
% e \textit{uxLength}. A primeira armazenar o tamanho da mensagem da
% fila e a outra armazenar a quantidade de mensagem que a fila pode
% receber.
...
As funções relacionadas com as filas de mensagens são:

\begin{itemize}
  \item{xQueueCreate:} cria uma nova fila de mensagem;
  \item{xQueueSend:} enviar uma mensagem para uma fila de mensagens;
  \item{xQueueSendToBack:} envia uma mensagem para o final de uma
  fila de uma fila de mensagens.;
  \item{xQueueSendToFront:} envia uma mensagem para o início de uma
  fila de mensagem;
  \item{xQueueReceive:} ler e retirar a mensagem localizada no início
  da fila de mensagens;
  \item{xQueuePeek:} ler sem retirar a mensagem localizada no início da
  fila de mensagens;
  \item{vQueueDelete} Exclui uma fila de mensagens do FreeRTOS.
\end{itemize}

%% DD->SG: Acho que nesse ponto seria importante mostrar qual seriam
%% as outras etapas da modelagem, e como proceder à inclusão de novas
%% funcionalidades. Uma coisa que poderiamos comentar é que o
%% refinamento pode ser usado para complementar a especificação. A
%% alternativa seria mudar a máquina inicial e, caso fizermos isso,
%% teriamos que recomeçar do zero muitas provas.

\section{Modelagem}
\label{sec:modelagem}

%% DD->SG O que diabo são as permutações de uma tarefa? Assumi que era
%% mudança de estado...

A modelagem de um sistema complexo com o método B pode ser facilitada
se usamos técnicas como: 
\begin{enumerate}
\item Partes dos requisitos podem ser abstraídos da especificação
  ...
  ulteriormente através de refinamentos.
...
No caso da modelagem do FreeRTOS, usamos ambas táticas para construir
a especificação.  

%% DD: terminar aqui.

%% DD->SG: incluir diagrama de máquinas

Assim, com o intuito de iniciar a modelagem do
FreeRTOS utilizando a notação de máquina abstrata, foi desenvolvida
uma especificação abstrata dos conceitos e funcionalidades propostos
pelo projeto de modelagem.  Nessa especificação abstrata o principal
objetivo foi modelar os aspectos do sistema relacionados com o
gerenciamento de tarefas. Assim, as variáveis de estado do modelo
inicial tem como objetivo representar as tarefas criadas e cadastradas
no sistema e o estado de cada uma dessas tarefas. Cada função do
FreeRTOS relacionada com o tratamento de tarefas foi modelada através
de uma operação na máquina B.

Com isso, nessa primeira etapa da especificação inicial, as
funcionalidades e conceitos do FreeRTOS propostos na sessão
\ref{sec:projeto} foram especificados apenas com informações
relevantes para os estados de uma tarefa e suas mudanças de estado.

É importante destacar que, nesse modelo inicial, a modelagem do
escalonador foi propositalmente simplificada. Para cada função do
FreeRTOs em que uma nova tarefa podia ser escalonada para se executar,
a operação correspondente no modelo B determina a tarefa escolhida de
forma não determinística entre todas as tarefas prontas. Assim, o
conceito de prioridade de tarefas foi totalmente abstraído na
especificação do comportamento das operações \emph{nessa modelagem
  inicial}.

Porém, observamos na seção~\ref{sec:freertos} que, no FreeRTOS, a
tarefa em execução tinha que ter uma prioridade maior ou igual às das
tarefas prontas. A modelagem inicial que desenvolvemos claramente não
permite garantir isto. Foi então o propósito do nosso primeiro
refinamento introduzir restrições adicionais quanto à prioridade das
tarefas.

\subsection{Modelagem da característica de estado de uma tarefa}

A modelagem abstrata dos conceitos e funcionalidades do sistema
baseada na característica de estado de uma tarefa pode, assim como no
projeto de modelagem, ser dividida em dois grupos, grupo de tarefa e grupo
de fila de mensagens. No grupo de tarefa, como o próprio nome já diz,
estão presentes os modulos que especificam o conceito de tarefa e suas
funcionalidades e no grupo de fila de mensagens estão os módulos
responsáveis pelo conceito de fila de mensagens e suas
funcionalidades.

\subsubsection{Tarefa}

A modelagem mais abstrata do conceito de tarefas foi realizado na
máquina, $\mathit{Task\_Core}$ (seus trechos os mais relevantes estão
na figura~\ref{fig:tarefa_especificacao}). Nela, o conjunto abstrato
$\mathit{TASK}$ representa o tipo da entidade ``tarefa'' do FreeRTOS.
A variável de estado $\mathit{tasks}$ representa o conjunto de tarefas
que estão sendo gerenciadas pelo sistema. Além disso, são
identificados os conjuntos de tarefa $\mathit{running}$,
$\mathit{ready}$, $\mathit{blocked}$ e $\mathit{suspended}$, os quais
representam as tarefas que estão respectivamente no estado executando,
pronto, bloqueado e suspenso. 
...
Uma vez determinada, de forma bastante abstrata, o tipo dos estados e
caracterizado aqueles considerados válidos, foram especificadas
algumas operações básicas responsáveis por alterar as características
de uma tarefa, acrescentar ou eliminar uma tarefa, etc. Essas
operações básicas também estão presentes na máquina
$\mathit{Task\_Core}$, e são usadas para especificar as funcionaliades
do FreeRTOS relacionadas com o conceito de tarefas. Ao todo, foram
criadas 12 operações, das quais iremos observar apenas a operação
$\mathit{t\_create}$, responsável pela criação de uma nova
tarefa\footnote{O leitor interessado nas demais operações está
  convidado a visitar o repositório no endereço
...
esteja elemento do conjunto $\mathit{tasks}$, sendo, em seguida,
armazenada na variável $\mathit{task}$. Esse valor é resultado da
operação e as variáveis de estado $\mathit{tasks}$ e $\mathit{ready}$
são alteradas para também incluir esse valor.
...
Feita a modelagem das operações básicas do conceito de tarefa, resta
apenas especificar as funcionalidades que se utilizam desse
conceito. A especificação dessas funcionalidades é feita através da
máquina $\mathit{Task\_Core}$, na qual estão prensentes as
funcionalidades dos conjuntos controle de tarefa e controle do kernel
da sessão \ref{subsubsec:tarefa_funcionalidades}. Entretanto aqui
demonstraremos apenas a especificação da funcionalidade
$\mathit{xTaskCreate}$, a qual utiliza-se da operação básica
$\mathit{t\_create}$ para criar uma nova tarefa que sera gerenciada
pelo FreeRTOS. Assim, a especificação dessa funcionalidade foi feita
como demonstra a figura \ref{fig:xTaskCreate}, onde são identificados
dois comportamentos possíveis para a função: ou a tarefa é criada
através da chamada ao método $\mathit{t\_create}$ e uma referência
para a nova tarefa é retornada, junto com a mensagem de sucesso; ou, a
tarefa não é criada e uma referência nula, junto com a mensagem de
erro, é retornada.
...
Independente de qualquer abstração, para iniciar-se a especificação do
conceito de fila de mensagens é necessário que essa possua um conjunto
capaz de armazenar as mensagens que serão enviadas para a fila. Com
base nisso e na necessidade de preocupar-se como a característica de
estado de uma tarefa, o conceito de fila de mensagens foi especificado
pelo módulo $\mathit{Queue\_Core}$ como demonstra a
figura~\ref{fig:fila_especificacao}. Nela, inicialmente tem-se os
conjuntos abstratos $\mathit{QUEUE}$ e $\mathit{ITEM}$, que simbolizam
respectivamente o conjunto universo das filas de mensagens e suas
mensagens. Em seguida, a variáveis $\mathit{queues}$ é criadas para
armazenar as filas de mensagens que estão sendo gerenciadas pelo
FreeRTOS. E por fim, são criadas as relações $\mathit{queue\_items}$,
$ \mathit{queue\_sending}$ e $\mathit{queue\_receiving}$, responsáveis
por ligar uma fila de mensagens a um conjunto de itens, a um conjunto
de tarefas que esperam para enviar uma mensagem para a fila e a um
conjunto de tarefa que aguardam para ler uma mensagem da fila,
respectivamente.
...
responsáveis por manipular as características do conceito. Com isso
foram implementadas o total de seis operações, nas quais são feitas as
inclusões e exclusões de mensagens e tarefas nos conjuntos que formam
o conceito de fila de mensagens. E para finalizar essa especificação
abstrata, após a criação das operações básicas, as funcionalidades que
utilizam o conceito de fila de mensagens (funcionalidades do grupo
...
demais máquinas que se utilizam do conceito de tarefa fazem esse uso
...
adicionada a modelagem. Esse refinamento é feito através da máquina
...
adicionadas a definição do conjunto de prioridade,
$\mathit{PRIORITY}$, que até o momento tratava-se de um conjunto
abstrato, a variável $\mathit{t\_priority}$, usada para ligar uma
tarefa a sua prioridade e a restrição presente nas seis últimas linhas
do $\mathit{INVARIANT}$ a qual especifica que, após inicializado o
funcionamento do sistema a tarefa em execução deve possui prioridade
maior ou igual às tarefas de estado pronto.
