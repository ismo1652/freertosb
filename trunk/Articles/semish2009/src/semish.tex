...
são bastantes difundidas em produtos comerciais através de sistemas
computacionais embutidos.

A funcionalidade primordial do FreeRTOS é provê acesso a recursos de
\textit{hardware}, permitindo o desenvolvimento de sistemas concorrentes de
tempo-real. Assim, ele funciona como uma camada de abstração, que tem como papel
esconder dos desenvolvedores de aplicações os detalhes do hardware que será
utilizado. Mais precisamente, o FreeRTOS fornece para os seus usuários serviços
de gerenciamento de tarefa, de comunicação e sincronização entre tarefas, de
memória, de tempo-real e de controle dos dispositivos de entrada e saída. Vale
salientar que o FreeRTOS assume que a arquitetura alvo possui um único
processador.

Para o usuário ter acesso aos seus recursos o FreeRTOS disponibiliza
funcionalidades sob a forma de uma biblioteca de tipos e funções que são
linkeditada com o código da aplicação a ser desenvolvida. Tipicamente esse código
é divido em duas partes: na primeira parte é criado o código das tarefas que
serão executadas durante a operação do sistema, e na segunda, um código
responsável pela inicialização do sistema, o qual deverá cadastrar as tarefas e
dar início ao escalonador, é criado.
...
\item uma pilha na qual é armazeno o ambiente de execução
  (estados dos registradores) da tarefa quado está é interrompida;
  %% DD->DH: NO FREERTOS AS TAREFAS TEM UMA PILHA PARA ARMAZENAR O SEU CONTEXTO
  %% ACHO QUE FUI INFELIZ EM BOTAR UM NOME PARA A PILHA. A EXPLICAÇÃO DE TAL
  %% PILHA ESTÁ EM http://www.freertos.org/taskandcr.html 
...
Na configuração preemptiva, a tarefa em execução pode ser interrompida a qualquer
momento pelo escalonador e consequentemente substituída por uma outra tarefa de
acordo com a política de escalonamento, a qual é baseada em prioridades. Na
configuração cooperativa, a tarefa em execução não é interrompida pelo
escalonador, pois ela própria decide o momento de sua pausa seguida da sua
...
tarefa será bloquada até uma mensagem chegue à fila.
...
fenômeno de intertravamento de tarefas chamado de inversão de prioridade.
...
usando a análise estática. Finalmente as diferentes etapas de
...
qA análise estática dessa máquina demanda que seja verificado que ela é
...
É seguindo esse raciocínio que o método B constrói sistemas de uma forma mais
segura e coerente. Ele será o formalismo utilizado para desenvolver uma
especificação formal do FreeRTOS. O modelo assim obtido poderá ser usado das
seguintes formas:
\begin{itemize}
...
desenvolvimento incremental dessa modelagem. 
...
são que casos particulares de filas de mensagens e que tarefa é a entidade
...
Dois tipos de funções manipulam a entidade tarefas: gerenciamento de tarefa e
controle do escalonador. A seguir tem-se a listagem e descrição de cada uma das
funcionalidades desses grupos escolhidas para fazerem parte da modelagem inicial.
...
Para a manipulação da entidade de fila de mensagens existem as seguintes
funções:
...
Após determinadas as funcionalidades básicas do sistema foi definidos os vários
níveis de abstração em que essas podem ser especificadas. Assim para uma
modelagem progressiva e gerenciável do FreeRTOS, os níveis de abstração foram
organizados da seguinte maneira:

\begin{enumerate}
  \item \textbf{Estado de tarefa:} Nesse nível é levando em consideração
  principalmente o comportamento das funcionalidades em relação ao estado de
  uma tarefa. Nele também são especificados os estados do escalonador assim
  como o conceito de tempo, tratado como tick no FreeRTOS
  \item \textbf{Prioridade:} Através dessasabstração a característica de
  prioridade é adicionada ao sistema. Procupando-se principalmente em garantir
  que a tarefa em execução possui prioridade maior ou igual as tarefas de
  estado pronto
  \item \textbf{Capacidade da Fila de Mensagens:} Nessa abstração será
  acrescentada ao modelo o tamanho de uma fila de mensagem, podendo assim
  quando um tarefa pode ser ou não bloqueada por uma fila.
  \item \textbf{Troca dos conjuntos por sequência:} Para finalizar a abstração
  das funcionalidades básicas os estados implementados como conjunto serão
  substuidos por sequências de elemento, podem assim criar uma ordenação para
  os elementos do mesmo.
\end{enumerate}

Tendo um nível de abstração satisfatória para a verificação de várias do
sistema em relação as suas funcionalidades classificadas como básica e
necessário agora acrescentar ao sistema nova funcionalidades. Assim começará
acrecentando o conceito de semáforo e suas funcionalidades, sendo que tal
funcionalidades lista a seguir:

\begin{itemize}
\item \textbf{vSemaphoreCreateBinary:} Criar um semáforo
\item \textbf{xSemaphoreTake:} Solicita um semáforo
\item \textbf{xSemaphoreGive:} Libera o semáforo
\end{itemize} 

Assim o projeto de modelagem segue nesse sentido, definindo níveis de
refinamentos entre as funcionalidades e quando necessário acrescenta novas
funcionalidades ao modelo até que uma especificação sólida do FreeRTOS possa
ser feito e utilizada.
...
a especificação. Assim, essa especificação inicial foi divida em vários módulos
que serão explicados no decorrer da modelagem, ficando a arquitutera básica
dessas modelagem inicial organizada como mostra a
figura~\ref{fig:arquitetura_modulos} na qual temos o módulos básico Task\_Core e
Queue\_Core que são utilizados por módulos mais complexos como Task\_Basic e
Queue\_Basic, sendo esse último base para o módulo Queue.

\begin{figure}[!h]
\centering
\includegraphics[scale=0.7]{figuras/arquitetura.jpg}
