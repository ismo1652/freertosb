...
O método B trata-se de uma abordagem orientada a modelos para a especificação
de componentes de \textit{softwares}. Através dele é possível criar-se sistemas
coerentes e seguros que comprovadamente realizam as funcionalidades descritas
em sua especificação. Para isso ele utiliza-se da lógica de primeira ordem e da
teoria dos conjunto, as quais fornecem base teórica suficiente para suas
especificações possam ser provadas como coerentes.

Por ser uma abordagem orientada a modelos, o componente principal da sua
especificação é o módulo, também chamado de maquina (\textit{Machine} em inglês).
Esses módulos são criados através de uma notação denominada Notação de Máquina
Abstrata, na qual os aspectos do sistema são descritos através de partes bem
definidas da especificação. Por exemplo, a figura~\ref{fig:tarefas_maquina},
mostra o módulo \textit{Kernel} responsável por especificar um simples sistema que armazena e
exclui tarefas. Nesse módulo, podemos ver que o mesmo está dividido em seis
partes bem definidas \textit{MACHINE}, \textit{SETS}, \textit{VARIABLES}, \textit{INVARIANT},
\textit{INITIALISATION} e \textit{OPERATIONS}
...
\quad \mathbf{card}(\mathit{tasks})\leq 10\\
...
\quad\quad \mathit{card}(\mathit{tasks})<10\\
\quad \THEN\\
...
A parte \textit{MACHINE} informa que o arquivo é do tipo \textit{Machine}, ou,
Modulo\footnote{Em B, além do tipo módulo um arquivo pode ser do tipo Refinamento
ou Implementação}, e o seu nome é $\mathit{Kernel}$. Após isso, tem-se as parte
\textit{SETS}, onde são definidos os conjuntos abstratos utilizados pelo módulo,
nesse caso, o conjunto $\mathit{TASK}$, que simboliza o conjunto universo de
todas as tarefas que podem ser manuseadas pelo sistema. Em, seguida nas partes
\textit{VARIABLES} e \textit{INVARIANT} são definidos respectivamente os
estados armazenados pelo sistema, conhecidos como variáveis, e suas restrições.
Assim, nesse exemplo temos a variável, $\mathit{tasks}$, responsável por
armazenar as tarefas adicionadas ao sistema, sendo que sua capacidade máxima é
definida como dez tarefas. E por fim temos as partes de \textit{INITIALISATION},
onde são colocados os estado iniciais do módulo, e \textit{OPERATIONS}, onde são
descritas as funcionalidades realizadas pelo sistema denominadas operação. È
através das operações que o usuário manipula os estados do módulo, no nosso caso,
incluem e excluem tarefas no sistema.

Agora para garantir a corretude dessa especificação é necessário provar que as
partes que compõem o módulo estão coerente entre si, ou seja, nunca uma operação
da máquina ira quebra as restrições impostas ao estado $\mathit{tasks}$. Essa
coerência é garantida através das obrigações de prova geradas apartir do modelo.
Um exemplo de uma obrigação de prova desse modelo pode ser visto a seguir, nela
\textit{INVARIANT} da máquina, presente na parte de baixo da prova, deve ser
mantido quando a operação $\mathit{tasks}:=\mathit{tasks}\cup \{\mathit{task}\}$
for executada. Sendo essa facilmente provada.
\begin{small}
$$
\begin{array}[t]{ccc}
\forall \mathit{task}.(\mathit{task} \in \mathit{TASKS} \land
\mathbf{card}(\mathit{tasks}) < 10 \land
\mathit{task} \not\in  \mathit{tasks}
\Rightarrow \\
\lceil \mathit{tasks}:=\mathit{tasks}\cup \{\mathit{task}\} \rceil
(\mathit{task}\in \mathit{TASK} \land 
\mathit{task}\not\in \mathit{tasks} \land
\mathit{card}(\mathit{tasks})<10)  
)
\begin{array}[t]{l}
\end{array}
\end{array}
...
Entratanto, modelos abstratos são difíceis de serem transformados em código fonte
executáveis. Para que uma especificação torne-se mais detalhada e mais perto de
uma especificação implementação o método B proporciona o mecanismo de
refinamento. Nesse mecanismo, a especificação abstrata é refinada até nível de
algoritmo, podendo esse também ser refinado para o nível de assembler da máquina
alvo. Assim cada refinamento adcionas detalhes a especificação abstrata que a
torma mais próxima do implementável. Um exemplo de refinamento da nossa máquina
de criar tarefas pode ser visto na figura \ref{fig:tarefas_maquina_r}. Nela a
variável de tarefas $\mathit{tasks}$ representada por $\mathit{tasksR}$, não é
mais tratada como um conjunto de tarefa, e sim como uma sequência de tarefas,
estrutura mais fácil de ser implementada do que um conjunto.
...
\quad \mathgf{ran}(\mathit{tasks\_r})=\mathit{tasks}\\
...
É seguindo esse raciocínio que o método B especifica constrói sistemas de uma
forma mais segura e coerente. Ele será o formalismo utilizado para especificar
ferramenta FreeRTOS tornando-a coerente com sua funcionalidade, proporcionando
assim uma ferramenta capaz de construir sistemas de tempo real mais fidedignos.
