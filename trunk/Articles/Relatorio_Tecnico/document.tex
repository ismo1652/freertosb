...
Como mostrado na sessão \ref{sec:freertos} o FreeRTOS possui uma enorme
...
mensagem (sessão \ref{subsec:fmensagem}). As funcionalidades da API que utilizam
esses conceitos escolhidas, para o melhor entendimente e gerenciamento, foram
divididas em cinco grupos: criação de tarefa; controle de tarefa; controle do
kernel; utilidades de tarefas e gerenciamento de fila. Em seguida, tem-se o
detalhamento de cada grupo junto com suas funcionalidades que os compoem.

\subsection{Criação de Tarefa}
\label{subsec:tcreation}

Nesse grupo, estão presente as funcionalidades responsáveis pela criação,
exclusão e referênciamente de uma tarefa. Ele é formado pela seguintes
operações:
...
\subsection{Gerenciamente de fila de mensagem}
...
% conceito de tarefa, as características de prioridade, troca de contexto e
% código de execução foram abstraídas da modelagem inicial, sendo especificadas
% apenas a característica de estado e suas mudanças ]
