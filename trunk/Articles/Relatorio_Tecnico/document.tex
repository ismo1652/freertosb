...
Jim and information base for outhers works that intent specific formally the
FreeRTOS.
\end{abstract}
...
eficiencia é largemete utilizado em vários tipos de aplicações. Para cumprir
esse objetivo, foi criada, utilizando-se o método B, uma modelagem abstrado
...
Sistema operaciona de tempo real, FreeRTOS, Desafio, Método B 
\end{palavras-chave}
%----------------------------------------------------------RESUMO %
...
seguida substituida por outra tarefa. Diferentimente, no tipo cooperativo, o
...
importância da tarefa para o sistema; um estado que desmonstra a atual
...
A prioridade de uma tarefa pode váriar de 0 (zero)até uma constante de prioridade
...
resalta, é que tarefas de mesma prioridade dividem o tempo de execução entre si.
...
não possuem pilha de contexto e, conseguentemente, não possuem contexto próprio, ou
...
estado ``suspenso''. Conseguentemente, o gráfico de transição de estados de uma
...
interrupção. Assim, resta para o escalonador apenas a tarefa de descidir qual
...
lida, caso contrário, a tarefa é bloqueda até a chegada de uma mensagem na fila.
...
o semâforo do recurso, caso este esteja liberado, a tarefa tem a permição de
...
o semâforo tem, mometaneamente, a sua prioridade elevada, para que assim ela
possar realizar a suas funções sem interrupções e, conseguentemente, liberar mais
...
gerênciar, iniciar e finalizar as atividades do kernel. Nesse grupo estão
...
\begin{itemize}
  ...
  Entretando, o espaço oculpado pela a mesma só será liberado pela tarefa
  ociosa.
...
todo foram criados 7 máquinas, das quais 2 pertecem ao grupo de tarefas e 3
pertecem ao grupo de fila de mensagens, restando 1 máquina para os parâmetros
...
seguinda, a variável $\mathit{tasks}$ é instituida para amazenar todas as tarefas
...
$\mathit{tasks}$. Em seguinda, essa nova tarefa é finalmente adicionada ao a
...
apenas por futuros refinamentos da operação para descidir o estado inicial da
...
ser vistas no reposiótirio da modelagem em \cite{repository}.
...
da disponibiliade de memória, característica não tratada nessa modelagem inicial.
...
mesagem para a fila e um conjunto de tarefas que aguardam para receber uma mensagem da
...
seguida, a variávei $\mathit{queues}$ é criadas para armazenar todas as filas de
...
conjunto de itens, a um conjunto de tarefas que esparam para enviar uma mensagem
...
conjunto de tarefa que aguardam por uma mensagem da fila. Tendo esses parâmentros
...
uma fila de mensagem e desbloquar uma tarefa que aguarda por uma mensagem da
fila, bloquear a tarefa remetente, caso a fila esteja cheia, ou retorna uma
mesangem de erro, caso a mensagem não possa ser enviada e a tarefa não possa ser
...
$\mathit{DelayTask}$. Por fim, no último comportamente, apenas uma messagem de
...
fila de mensagens. A especificação competa dessa funcionalidade pode ser vista
...
função e retorna o resultado da operação, dizendo essafoi realizada com
...
difículdades com as obrigações de prova, pois sempre que tentava-se fazer uma
modelagem mais complexa não conseguia-se provar todas as obrigações de provas
geradas pela modelagem.

Como conseguência das dificuldades com as obrigações de prova, foi necessário
...
modelagem, como prioridade e tamanho de uma fila de mensagem, seriam excluidas da
modelagem inicial, restanto apenas as características de estado de uma tarefa e
os conjuntos que compoem uma fila de mensagens.
...
não oferece suporte para especificar eventos concorrêntes, como ocorre com as
...
semalhança muito grande entre os dois formalismo B.
